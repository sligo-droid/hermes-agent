...
- **Descriptions:** rows show compact summaries by default. When the read model has richer request/proposal context, expose a per-row full-description toggle so operators can inspect the complete plain-text context without opening the worker ticket.
