...
Last updated: 2026-06-20 09:22 UTC
...
| Gateway runtime heartbeat diagnostics | implemented | `hermes cron status/list` and `hermes doctor` now distinguish a fresh profile-scoped `gateway_state.json` heartbeat from visible PID/service evidence, so sandboxed/PID-namespaced diagnostics no longer report the default gateway stopped when the live gateway heartbeat is running. |
