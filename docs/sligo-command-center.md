...
Project tabs at the top of the Command Center scope this same ledger by project. The tabs are populated from configured project metadata plus discovered snapshot projects, not hard-coded per view. Current visible projects are Hermes (`#dev`), PID, and examine; known stale project keys are explicitly blocklisted so future discovered projects can appear unless blocklisted. Filtering by project keeps Sources as intake, Work Items as the status ledger, and worker boards/Kanban task runs as child execution artifacts.
