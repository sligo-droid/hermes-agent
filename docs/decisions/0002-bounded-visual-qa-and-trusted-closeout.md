...
- Concrete contract viewport dimensions govern the serialized deterministic-check, screenshot, retry-fingerprint, and restoration transaction. Hermes restores and verifies the exact supervisor-owned prior viewport state; inability to apply, verify, reapply, or restore that state is non-passing evidence.
