...
<table>
<tr><td><b>真正的终端界面</b></td><td>交互式 CLI，支持多行编辑、斜杠命令自动补全、对话历史、中断重定向和流式工具输出。</td></tr>
...
Hermes 有两种入口：用 `hermes` 启动交互式 CLI，或运行网关从 Telegram、Discord、Slack、WhatsApp、Signal 或 Email 与之对话。进入对话后，许多斜杠命令在两种界面中通用。
