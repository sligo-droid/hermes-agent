...
A worked example of the dead-grant and revoked-session branches in
[OAuth MCP troubleshooting](troubleshooting-and-security.md): a provider whose
OAuth session dies on a recurring basis, where the durable fix is to drop OAuth
for a static API key.
...
mcp_servers:
  stripe:
    ...
    headers:
      Authorization: "Bearer rk_live_..."  # restricted key from Dashboard
...
Then `/reload-mcp` (full restart only if the breaker remains tripped after
reload, per the linked troubleshooting reference).
