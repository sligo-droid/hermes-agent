...
class GatewayStreamConsumer:
    ...
    async def run(self) -> None:
        ...
        # Rich-capable adapters can safely keep a response whole above the
        # legacy edit limit. Use the adapter's explicit overflow capability
        # instead of fragmenting at MAX_MESSAGE_LENGTH unconditionally.
        ...
        try:
            while True:
                ...
                if should_edit and self._accumulated:
                    ...
                    while (
                        ...
                        # This chunk is sealed once we reset ``_message_id``
                        # below, so apply final platform formatting now. It is
                        # not the turn-final answer, though: later chunks still
                        # need to land before final delivery is complete.
                    ...
                    current_update_visible = await self._send_or_edit(
                        ...
                        # turn-final answer; only got_done marks delivery.
                ...
                if got_done:
                    ...
                    if self._accumulated:
                        if self._fallback_final_send:
                            ...
                            # A fresh-final send already delivered the answer.
                            self._final_content_delivered = True
                        elif (
                            ...
                            # final accumulated content. A split edit carried
                            # finalize=True itself and adopted its last
                            # continuation, so re-finalizing the full text
                            # would duplicate the overflow chunks.
            ...
            # Best-effort final edit on cancellation. Apply final platform
            # formatting, but let this handler own the delivery flags.
    ...
    async def _try_fresh_final(
        ...
        ``is_turn_final`` is False when finalizing an interim segment at a
        tool boundary; in that case the final-delivery flag stays unset.
