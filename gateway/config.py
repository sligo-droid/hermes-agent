    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.channel_overrides:
            result["channel_overrides"] = {
                cid: override.to_dict()
                for cid, override in self.channel_overrides.items()
            }
        return result
...
class GatewayConfig:
    ...
    def get_unauthorized_dm_behavior(self, platform: Optional[Platform] = None) -> str:
        ...
        if platform:
            ...
            # Email is inbox-shaped and should silently ignore unknown senders
            # unless the operator explicitly opts that platform into pairing.
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            for plat in _shared_loop_targets:
                ...
                # Support the same shared-key bridge when the platform is
                # configured only under ``platforms`` / ``gateway.platforms``.
                # Explicit ``enabled`` precedence remains owned by the merge
                # above, so only a top-level block may re-apply it here.
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    try:
        ...
        for entry in platform_registry.plugin_entries():
            ...
            # Preserve an explicit disabled setting even when credentials are
            ...
            if existing_cfg is None or not existing_cfg.enabled:
                if entry.is_connected is not None:
                    ...
                    if not configured:
                        ...
                        continue
            # Dependency checks can lazy-install SDKs, so only probe them for
            # an explicitly enabled or credential-confirmed platform.
            try:
    ...
    if existing_relay is not None:
        relay_url_yaml = str(
            existing_relay.extra.get("relay_url")
            or existing_relay.extra.get("url")
            or ""
        ).strip()
    relay_url = relay_url_env or relay_url_yaml
    if relay_url:
        relay_config = _enable_from_env(Platform.RELAY)
        relay_config.extra["relay_url"] = relay_url.rstrip("/")
