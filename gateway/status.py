...
def compute_gateway_source_identity(
    *,
    ...
    """Resolve immutable build metadata or the loaded source checkout."""
    ...
    try:
        ...
        root = (
            Path(root_result.stdout.strip()).resolve(strict=False)
            if root_result.returncode == 0 else fallback_root
        )
        ...
        dirty = (
            ...
            or bool((status_result.stdout or "").strip())
            or not commit
        )
