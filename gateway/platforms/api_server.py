...
def _hermes_version() -> str:
    """Return the installed or in-tree Hermes version without raising."""
    try:
...
except Exception:  # pragma: no cover - optional defense-in-depth
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Maps incoming ``model`` values to per-request provider/model config.
        # Invalid entries are ignored so a typo cannot prevent server startup.
    ...
    def _parse_model_routes(raw: Any) -> Dict[str, Dict[str, Any]]:
        ...
        Route API keys are upstream provider credentials, not caller auth, and
        must never be exposed through discovery or logs.
        """
    ...
    def _resolve_route(self, model_alias: Any) -> Optional[Dict[str, Any]]:
        """Return the model route for *model_alias*, if configured."""
    ...
    def _session_model_override_for(
        ...
        """Return an explicit session ``/model`` override, if present."""
        if not session_key:
            return None
        try:
            from gateway.run import _gateway_runner_ref
    ...
    def _create_agent(
        ...
        ``route`` may override the global provider/model for this request. An
        explicit session ``/model`` override takes precedence over the route.
        """
        ...
        # A fallback provider can resolve a model alongside the other runtime
        # kwargs. Remove it before the explicit AIAgent ``model`` argument to
        # avoid duplicate-key construction failures.
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        if provided_session_id:
            ...
            # Reject header injection and filesystem traversal shapes before
            # the ID reaches session snapshots or request-dump paths.
    ...
    async def _write_sse_chat_completion(
        ...
        try:
            ...
            # Get usage from completed agent. A clean queue termination does
            # not imply the agent itself succeeded: it may raise after its last
            # delta or return a failed/partial result. Mirror the non-streaming
            # finish-reason contract so clients never see a false "stop".
            ...
            if finish_reason != "stop":
                if err_msg:
                    safe_err = _redact_api_error_text(err_msg)
                    finish_chunk["error"] = {
                        "message": safe_err,
    ...
    async def _run_agent(
        ...
        ``route`` is a per-request model/provider override resolved from the
        incoming API model alias.
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        # Share the configured cap and live-work accounting with the other
        # agent-serving endpoints and shutdown drain logic.
        limited = self._concurrency_limited_response()
        if limited is not None:
            return limited
        ...
        # Conversation/session identifiers are memory scopes, not approval
        # authorization namespaces. Keep concurrent approvals isolated by run.
    ...
    async def disconnect(self) -> None:
        """Stop the aiohttp web server and release owned resources."""
