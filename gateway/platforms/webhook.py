...
# names a profile this gateway does not serve.
...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _resolve_secret(self, route_config: dict, route_name: str = "") -> str:
        ...
        if "secret" in route_config:
            return str(route_config.get("secret") or "")

        if "secret_env" in route_config:
            route_secret_env = route_config.get("secret_env") or ""
            if not route_secret_env:
                return ""
    ...
    def _resolve_request_profile(self, request: "web.Request"):
        """Resolve and validate an optional /p/<profile>/ URL prefix."""
    ...
    async def on_processing_complete(
        ...
        """Close the one-shot webhook session after its run finishes."""
