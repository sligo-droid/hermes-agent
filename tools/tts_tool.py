    ...
    *,
    ...
    if output_path:
        ...
        # otherwise fall back to .mp3 (Edge TTS will attempt ffmpeg conversion later).
            ...
            "openai", "deepinfra", "elevenlabs", "mistral", "gemini",
        }:
            file_path = out_dir / f"tts_{timestamp}.ogg"
        else:
            file_path = out_dir / f"tts_{timestamp}.mp3"

    # Some providers always synthesize MP3/WAV and rely on the post-generation
    # Telegram conversion below. If a caller explicitly requests ``.ogg``
    # (for example the gateway's native Telegram voice bubble), render to the
    # usual MP3 intermediate first so ffmpeg does not receive mislabeled input
    # or the same path for both input and output.
    if (
        want_opus
    ...
    try:
        ...
        elif provider in {"elevenlabs", "openai", "deepinfra", "mistral", "gemini"}:
