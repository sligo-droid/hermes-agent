...
# so external module-attribute access still works.

INDEX_CACHE_TTL = 3600  # 1 hour

# ``unittest.mock.patch`` / pytest monkeypatch first read dynamic compatibility
# attributes through ``__getattr__`` and may restore that concrete Path as a
# real module attribute afterward. Remember values produced by the dynamic
# fallback so a later profile switch can discard such stale restorations while
# still honoring an intentional, different test override.
_DYNAMIC_PATH_FALLBACK_VALUES: Dict[str, Dict[Path, Path]] = {}
...
def _override(name: str):
    ...
    try:
        ...
        return forced
    fallback_home = _DYNAMIC_PATH_FALLBACK_VALUES.get(name, {}).get(forced_path)
    if fallback_home is not None and fallback_home != get_hermes_home():
...
def __getattr__(name: str):
    ...
    value = resolver()
    _DYNAMIC_PATH_FALLBACK_VALUES.setdefault(name, {})[Path(value)] = get_hermes_home()
    return value
...
_BUNDLE_BINARY_MAX_FILE_BYTES = 1024 * 1024
_BUNDLE_TEXT_SUFFIXES = frozenset(
    {
        ...
        ".py", ".sh", ".bash", ".zsh", ".fish", ".ps1", ".env", ".css",
        ".scss", ".less", ".svg", ".sql", ".lock",
    }
)
...
def _bundle_text_content(rel_path: str, content: Union[str, bytes]) -> Optional[str]:
    ...
    binary_asset = (
        bool(path.parts)
        and path.parts[0].lower() == "assets"
        and name not in _BUNDLE_TEXT_NAMES
        and suffix not in _BUNDLE_TEXT_SUFFIXES
    )
    text_candidate = not binary_asset
    if isinstance(content, str):
        ...
        if "\x00" in content:
            raise ValueError(f"Skill bundle text file contains NUL bytes: {rel_path}")
        return content
    if len(content) > _BUNDLE_SCAN_MAX_FILE_BYTES:
        if text_candidate:
            raise ValueError(f"Skill bundle text file exceeds policy scan limit: {rel_path}")
        if len(content) > _BUNDLE_BINARY_MAX_FILE_BYTES:
            raise ValueError(f"Skill bundle binary file exceeds policy size limit: {rel_path}")
        return None
    if b"\x00" in content:
        if text_candidate:
            raise ValueError(f"Skill bundle text file contains NUL bytes: {rel_path}")
...
# searchable and filterable without disturbing source-based deduplication.
...
_PROVIDER_FILTER_VALUES = frozenset(
    provider.lower() for provider in GITHUB_TAP_PROVIDERS.values()
)
...
def _filter_results_by_provider(
    ...
    return [
        ...
        if str((result.extra or {}).get("provider", "")).lower() == wanted
    ]
...
def quarantine_bundle(bundle: SkillBundle) -> Path:
    ...
    ensure_hub_dirs()
    return materialize_bundle(bundle, _quarantine_dir())
...
class HermesIndexSource(SkillSource):
    ...
    def search(self, query: str, limit: int = 10) -> List[SkillMeta]:
        """Search the cached index and rank all matching metadata."""
        index = self._ensure_loaded()
        ...
        for index_position, skill in enumerate(skills):
            name = str(skill.get("name", "")).lower()
            provider = str(
                (skill.get("extra") or {}).get("provider", "")
            ).lower()
            searchable = " ".join(
                [
                    name,
                    str(skill.get("description", "")).lower(),
                    " ".join(str(tag).lower() for tag in skill.get("tags", [])),
                    str(skill.get("identifier", "")).lower(),
                    provider,
                ]
            )
            ...
            if name == query_lower:
                ...
                score = 5
            scored.append((score, index_position, skill))

        scored.sort(key=lambda item: (item[0], item[1]))
        return [self._to_meta(skill) for _, _, skill in scored[:limit]]
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    # Provider labels such as "nvidia" identify GitHub taps represented by
    # the centralized index; they are not source ids themselves.
    provider_filter = source_filter.strip().lower() in _PROVIDER_FILTER_VALUES
    effective_filter = "all" if provider_filter else source_filter
    ...
    for src in sources:
        sid = src.source_id()
        if effective_filter != "all" and sid != effective_filter and sid != "official":
            continue
