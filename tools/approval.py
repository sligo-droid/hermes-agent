...
def _is_cron_execution() -> bool:
    """Prefer scoped cron identity, preserving legacy env-only processes."""
    try:
        from gateway.session_context import is_cron_execution

        return is_cron_execution()
    except Exception:
...
def _looks_like_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* is a single-file Hermes cleanup attempt."""
    try:
        argv = shlex.split(command, posix=True)
    except ValueError:
        return False
...
def detect_dangerous_command(command: str) -> tuple:
    ...
    if _looks_like_verification_artifact_cleanup(command):
        description = "delete verification artifact outside canonical temp directory"
