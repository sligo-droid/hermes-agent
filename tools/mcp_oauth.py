...
def _raise_if_non_interactive(lead: str) -> None:
    """Fail fast with the shared interactive-login recovery guidance."""
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} Run `hermes mcp login <server>` interactively to "
            "(re)authorize, then restart or reload the gateway."
        )


@contextmanager
def force_interactive_oauth():
    """Treat the current GUI-driven OAuth context as interactive."""
...
def suppress_interactive_oauth():
    """Disable interactive OAuth across thread-safe coroutine scheduling."""
