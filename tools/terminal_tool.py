...
For GitHub Actions queries, discover workflows from the checked-in `.github/workflows/` files or `gh workflow list` before filtering runs. Prefer the stable workflow filename/path or numeric ID supported by the command; do not guess a display name.
...
def _get_session_terminal_cwd(default: str = "") -> str:
    """Return task-local gateway cwd when present, then TERMINAL_CWD/default."""
    try:
        from gateway.session_context import get_session_env
...
def _get_env_config() -> Dict[str, Any]:
    ...
    # for CLI/cron and older callers. Sanity-check it for container backends.
    ...
    if env_type == "docker" and mount_docker_cwd:
        docker_cwd_source = _get_session_terminal_cwd(_safe_getcwd())
...
def terminal_tool(
    ...
    try:
        ...
        if background:
            ...
            if conflict_note:
                logger.warning("background process: %s", conflict_note)
            ...
            if notify_on_complete or watch_patterns:
                ...
                if not _async_ok():
                    ...
                    if watcher_platform:
                        ...
                        if watcher_platform == "discord":
                            origin_work_item_id = get_trusted_discord_work_item_id()
