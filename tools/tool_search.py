...
* Tiered disclosure activates automatically when deferrable MCP/plugin schemas
  reach the configured context threshold. What scales with catalog size is the
  embedded listing:
    - Tier 0 — below threshold: pure passthrough, everything eager.
    - Tier 1 — deferred tools whose catalog listing fits the listing budget
      (``min(threshold_pct`` of context — default 10% — ``, listing_max_tokens)``):
...
class ToolSearchConfig:
    ...
    # Activation threshold and listing budget as a percentage of the model's
    # context window. Once active, it bounds how much context the embedded
...
def should_activate(
    ...
    ``"off"`` skips unconditionally. ``"on"`` activates when at least one
    deferrable tool exists. ``"auto"`` activates when deferred schemas meet
    the configured percentage of context, or a fixed 20K-token fallback when
    the active context length is unknown.
    """
