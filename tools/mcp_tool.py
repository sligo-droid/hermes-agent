...
try:
    ...
    # Elicitation types are gated separately so older MCP SDK versions keep
    # working while newer servers can request structured user approval.
    try:
...
class NonMcpEndpointError(ConnectionError):
    ...
    A genuine Streamable-HTTP endpoint responds with ``application/json`` or
    ``text/event-stream``.  A different content type on a successful response
    is non-retryable and normally means the configured URL points at a web UI.
    """
...
def _format_elicitation_schema_summary(schema: dict, server_name: str) -> str:
    """Render an elicitation schema as a concise approval description."""
...
class ElicitationHandler:
    """Route MCP ``elicitation/create`` requests through Hermes approvals.

    Form-mode requests are fail-closed and run the synchronous approval flow
    in a worker thread so the MCP event loop remains responsive. URL-mode
    requests are declined until Hermes has a safe out-of-band completion flow.
    """
    ...
    def session_kwargs(self) -> dict:
        """Return the callback kwargs accepted by ``ClientSession``."""
        return {"elicitation_callback": self}

    async def __call__(self, context, params):
        """Handle one SDK elicitation callback."""
        self.metrics["requests"] += 1

        if getattr(params, "mode", "form") == "url":
            logger.info(
                "MCP server '%s' requested URL-mode elicitation; declining",
                self.server_name,
            )
...
class MCPServerTask:
    ...
    __slots__ = (
        ...
        "_sampling", "_elicitation", "_registered_tool_names", "_auth_type", "_refresh_lock",
    ...
    async def _keepalive_probe(self) -> None:
        """Probe session liveness, falling back when optional ping is absent."""
        if not self._ping_unsupported:
            try:
                ...
                logger.info(
                    "MCP server '%s' does not implement ping; using list_tools keepalive",
                    self.name,
                )
    ...
    async def _run_stdio(self, config: dict):
        ...
        try:
            ...
            logger.warning(
                "MCP server '%s': OSV malware preflight timed out after %.0fs; proceeding",
                self.name,
    ...
    def _deregister_tools(self) -> None:
        """Remove this server's registered tools idempotently."""
        from tools.registry import registry
...
def _filter_mcp_children(pids: set) -> set:
    """Filter unrelated gateway children from a stdio spawn PID delta."""
    if not pids:
        return pids
    try:
        import psutil
    ...
    def _send_signal(pid: int, sig: int, server_name: str) -> None:
        ...
        if pgid is not None and killpg is not None:
            if _my_pgid is not None and pgid == _my_pgid:
                logger.warning(
                    "MCP server '%s' shares gateway pgid %d; using per-pid signal",
                    server_name,
                    pgid,
                )
            else:
...
def _stop_mcp_loop_if_idle() -> bool:
    ...
    ``_servers``. They should clean up an otherwise-idle loop, but must not tear
    down the process-global loop while live agent tools are registered on it.
    """
