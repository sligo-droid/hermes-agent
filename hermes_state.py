...
def _is_background_review_harness_message(msg: Dict[str, Any]) -> bool:
    ...
    return isinstance(content, str) and any(
        ...
        for prefix in _REVIEW_HARNESS_PREFIXES
    )
...
class SessionDB:
    ...
    def _insert_message_rows(
        ...
        """Insert messages as fresh active rows inside an existing transaction."""
        now_ts = time.time()
        inserted = 0
        tool_calls_total = 0
        for msg in messages:
    ...
    def replace_messages(
        ...
        *,
        ...
        Pass ``active_only=True`` to preserve soft-archived rows created by
        in-place compaction or rewind while replacing only the live transcript.
        """
    ...
    def resolve_resume_session_id(self, session_id: str) -> str:
        ...
        # so delegation / branch children never hijack the resume. This is the
        # fix for the desktop "I came back and the reply isn't there" report on
        # large sessions.
        try:
    ...
    def search_messages(
        ...
        Compaction-archived rows (``active=0``, ``compacted=1``) remain
        discoverable. Pass ``include_inactive=True`` to search every row.
        """
        if not self._fts_enabled:
            return []
...
class AsyncSessionDB:
    """Async facade that keeps every SessionDB call off the event loop."""
