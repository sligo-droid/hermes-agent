...
def _process_user_home() -> Path | None:
    """Return the OS account home independently of a child ``HOME`` value."""
    try:
        import pwd
...
def _safe_path_matches(path: Path, predicate) -> bool:
    """Run a pathlib existence predicate without leaking permission errors."""
    try:
...
def get_github_cli_config_dir(env: dict | None = None) -> str | None:
    """Return an authenticated GitHub CLI config directory when available.

    Worker and terminal subprocesses may deliberately use an isolated HOME.
    Preserve access to the operator's existing ``gh`` authentication without
    copying credentials into the worker home.
    """
    ...
    real_home = str(get_real_home(source_env) or "").strip()
    if real_home and real_home != "/tmp":
        candidates.append(Path(real_home).expanduser() / ".config" / "gh")
    ...
    for candidate in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if _safe_path_matches(candidate / "hosts.yml", Path.is_file):
            return key
    return None
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    ...
    real_home_path = (
        process_home
        if process_home is not None
        else Path(real_home) if real_home and real_home != "/tmp"
        else Path.home()
    )
    real_home_paths = (
        ("GIT_CONFIG_GLOBAL", real_home_path / ".gitconfig", Path.is_file),
        ("DOCKER_CONFIG", real_home_path / ".docker", Path.is_dir),
        ("CODEX_HOME", real_home_path / ".codex", Path.is_dir),
        ("CLOUDSDK_CONFIG", real_home_path / ".config" / "gcloud", Path.is_dir),
        ("NPM_CONFIG_USERCONFIG", real_home_path / ".npmrc", Path.is_file),
    )
    for key, path, exists in real_home_paths:
        if key not in env and _safe_path_matches(path, exists):
            env[key] = str(path)
...
def normalize_reasoning_effort(effort) -> str:
    """Normalize user/config reasoning text while preserving 0.19 tiers.

    Local routing continues to choose ``xhigh`` as its automatic spillover
    ceiling, but explicit ``max`` and ``ultra`` values remain available for
    providers and per-model overrides that support them.
    """
