...
def run_conversation(
    ...
    # API-only /goal state. Keep the persisted transcript clean: the goal
    # manager is control-plane state, not user-authored prompt text.
    if agent.session_id and isinstance(user_message, str):
        try:
            from hermes_cli.goals import agent_goal_context

            _goal_context = agent_goal_context(agent.session_id)
        except Exception:
            _goal_context = ""
        if _goal_context:
            if agent._persist_user_message_override is None:
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        try:
            ...
            if assistant_message.tool_calls:
                ...
                # 20-30K tokens the messages-only estimate misses (#14695).
