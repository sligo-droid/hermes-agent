...
# Short-lived positive-result cache for local/Ollama context probes. Startup
# resolves the same model from several call sites; sharing this cache avoids
# issuing duplicate requests while still allowing failed probes to retry.
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # DashScope/Alibaba reports the legal output range directly.
    ...
    # vLLM reports both the context window and prompt size in tokens.
...
def _query_local_context_length(model: str, base_url: str, api_key: str = "") -> Optional[int]:
    """Query a local server for context length, with a short positive TTL cache."""
    import time as _time
...
def _query_local_context_length_uncached(
    model: str,
    base_url: str,
    api_key: str = "",
) -> Optional[int]:
...
def get_model_context_length(
    ...
    1c. Endpoint-scoped metadata for models validated on one multiplexed endpoint
    ...
    # can make every hostname helper raise. Treat them as an unknown endpoint
    # so context resolution can fall back safely and the inference layer can
    # report the configuration problem itself.
    if base_url:
        try:
    ...
    # MoA exposes a virtual preset name and loopback base URL; neither carries
    # a meaningful context window. The aggregator is the acting model, so
    # resolve its real provider/model runtime and mirror that context length.
    ...
    # vLLM is running at a lower ``--max-model-len`` (for example 32768).
...
async def get_model_context_length_async(
    ...
    """Resolve context length without blocking an asyncio event loop.

    The synchronous resolver owns the full provider-aware chain, including
    explicit/custom-provider overrides, endpoint probes, persistent metadata
    caching, and provider error fallbacks. Offloading that exact call keeps
    gateway message preparation responsive without creating a divergent async
    implementation.
    """
    import asyncio
