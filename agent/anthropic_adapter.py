...
Foundry, CLIProxyAPI, and Palantir Foundry endpoints follow this pattern.
    """
    normalized = _normalize_base_url_text(base_url)
    if not normalized:
        return False
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to Hermes's outer loop, which honors Retry-After.
        # The SDK default would retry internally before that loop can apply
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # loop. The SDK default retries internally and can double-submit a
        # request before the outer loop observes Retry-After.
        "max_retries": 0,
    }
    if normalized_base_url:
...
def build_anthropic_bedrock_client(region: str):
    ...
    return _anthropic_sdk.AnthropicBedrock(
        ...
        # Keep retries in Hermes's provider-aware outer loop.
        max_retries=0,
...
# Backward-compatible alias for fork callers that adopted the tuple name.
_OAUTH_TOKEN_ENDPOINTS = tuple(_OAUTH_TOKEN_URLS)
_OAUTH_TOKEN_URL = _OAUTH_TOKEN_URLS[0]
# The token endpoint rate-limits Claude Code/CLI and browser User-Agent
# prefixes. Claude Code itself exchanges credentials through axios, so use
# that non-throttled fingerprint for both login exchange and refresh.
...
def exchange_anthropic_oauth_token(
    ...
    *,
    ...
    headers = {
        "Content-Type": content_type,
        "User-Agent": user_agent or _OAUTH_TOKEN_USER_AGENT,
    }
    for endpoint in _OAUTH_TOKEN_ENDPOINTS:
        req = urllib.request.Request(
            endpoint,
...
def run_hermes_oauth_login_pure() -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        result = exchange_anthropic_oauth_token(
            exchange_data,
            timeout=15,
...
def _apply_assistant_cache_control_to_last_cacheable_block(
    ...
    """Attach a message-level cache marker to the final cacheable block."""
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    immediately following user message; a global ID match is insufficient.

    Mutates ``result`` in place.
    """
    # Validate each assistant tool-use turn against only the immediately
    ...
    for i, m in enumerate(result):
        ...
        if i + 1 < len(result):
            next_message = result[i + 1]
            if next_message.get("role") == "user" and isinstance(
        ...
        m["content"] = kept if kept else [
            {"type": "text", "text": "(tool call removed)"}
        ]

    # Remove results whose tool-use block did not survive adjacency validation.
