...
def _compose_bounded_user_api_content(
    agent: Any,
    *,
    content: Any,
    ...
    """Compose automatic context under the live model's request budget."""
    if not isinstance(content, str):
        return None
    parts: list[RuntimeContextPart] = []
    if ext_prefetch_cache and ext_prefetch_cache.strip():
        parts.append(RuntimeContextPart(
            label="memory prefetch",
            text=ext_prefetch_cache,
            fence="memory-context",
        ))
    if plugin_user_context and plugin_user_context.strip():
        parts.append(RuntimeContextPart(
            label="pre_llm_call hook output",
            text=plugin_user_context,
    ...
    rendered = render_runtime_context_parts(parts, budget_tokens=budget_tokens)
    if not rendered.text:
        return None
    if rendered.changed:
        logger.info(
            "automatic runtime context budget applied: budget=%s input=%s "
            "rendered=%s base_request=%s truncated=%s omitted=%s",
            rendered.budget_tokens,
            rendered.input_tokens,
            rendered.rendered_tokens,
            base_request_tokens,
            ",".join(rendered.truncated) or "-",
            ",".join(rendered.omitted) or "-",
        )
...
class TurnContext:
    ...
    ext_prefetch_cache: str = ""
    # Number of compression calls already spent during the turn prologue.
    preflight_compression_attempts: int = 0
...
def build_turn_context(
    ...
    *,
    ...
    preview_readiness = getattr(agent, "_preview_readiness", None)
    if preview_readiness is not None:
        preview_readiness.reset_for_turn()
    ...
    # API-only coding-worker guidance. The routing prefix steers the current
    # model call but must not leak into the persisted transcript.
    ...
    if isinstance(original_user_message, str) and isinstance(user_message, str):
        try:
            ...
            decision = assess_worker_routing(
                ...
                tool_available="delegate_coding_task" in agent.valid_tool_names,
                api_mode=getattr(agent, "api_mode", ""),
                cwd=session_cwd,
            )
            coding_worker_prefix = decision.guidance
            agent._coding_worker_required_this_turn = decision.required
        except Exception:
            coding_worker_prefix = ""
            agent._coding_worker_required_this_turn = False
        if coding_worker_prefix:
            if agent._persist_user_message_override is None:
                agent._persist_user_message_override = original_user_message
            user_message = f"{coding_worker_prefix}{user_message}"
    else:
        agent._coding_worker_required_this_turn = False

    # API-only /goal state. Goal-manager control-plane text is useful model
    # context, but it is not user-authored transcript content.
    if agent.session_id and isinstance(user_message, str):
        try:
            from hermes_cli.goals import agent_goal_context

            goal_context = agent_goal_context(agent.session_id)
        except Exception:
            goal_context = ""
        if goal_context:
            if agent._persist_user_message_override is None:
                agent._persist_user_message_override = original_user_message
            user_message = f"{goal_context}\n\n{user_message}"

    # ``user_msg`` is already appended so close-persistence can see the live
    # turn. Update that same dict with the API-facing variant; the persistence
    # override above restores the clean text at the turn boundary.
    user_msg["content"] = user_message
    ...
    if (
        ...
        _api_content = _compose_bounded_user_api_content(
            agent,
            content=_turn_user_msg.get("content", ""),
            messages=messages,
        ...
        _persist_override = getattr(agent, "_persist_user_message_override", None)
        _clean_content = (
            _persist_override
            if _persist_override is not None
            and (
                not isinstance(_live_content, list)
                or isinstance(_persist_override, list)
            )
            else _live_content
        )
        # API-only prefixes such as coding-worker guidance, /goal state, and
        # voice-mode instructions still need a sidecar when there is no memory
        # or plugin context to compose. Early crash persistence rewrites the
        # live ``content`` to the clean override before the first API call;
        # retaining the pre-rewrite bytes here keeps the wire payload intact.
        if (
            _api_content is None
            and isinstance(_live_content, str)
            and _live_content != _clean_content
        ):
            _api_content = _live_content
        if _api_content is not None and _api_content != _clean_content:
    ...
    return TurnContext(
        ...
        plugin_user_context=plugin_user_context,
        ext_prefetch_cache=ext_prefetch_cache,
        preflight_compression_attempts=_preflight_compression_attempts,
    )
