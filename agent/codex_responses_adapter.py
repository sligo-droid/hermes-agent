...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    ``is_xai_responses`` suppresses encrypted reasoning replay for the xAI
    OAuth surface.  Native Codex and compatible relays can replay opaque
    reasoning blobs, but xAI OAuth has rejected multi-turn replay payloads;
    visible assistant text remains in the input for conversational continuity.
    ...
    for msg in messages:
        ...
        if role in {"user", "assistant"}:
            ...
            if role == "assistant":
                # Replay encrypted reasoning items from previous turns on
                # Responses backends that support it. xAI OAuth is explicitly
                # excluded; its visible assistant text is still replayed.
                codex_reasoning = (
