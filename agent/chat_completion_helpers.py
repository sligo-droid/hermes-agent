...
# When a non-empty fallback chain is fully exhausted on a non-rate-limit
# failure, keep the next turn from immediately restoring the primary runtime
# and replaying the full chain. This short gate reuses the existing
# ``_rate_limited_until`` restoration guard without replacing the longer
# rate-limit/billing cooldown.
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        _oauth_tool_name_compat = bool(
            getattr(agent, "_anthropic_oauth_tool_name_compat", False)
        )
        ...
        return _transport.build_kwargs(
            ...
            oauth_tool_name_compat=_oauth_tool_name_compat,
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    if assistant_tool_calls:
        ...
        for tool_call in assistant_tool_calls:
            ...
            # Preserve executable tool arguments verbatim in conversation
            # history. Redacting arbitrary terminal/file arguments here can
            # change the command replayed to the model on the next turn.
            # Browser typing remains a special case because its input may be
            # echoed into persisted UI traces rather than replayed as code.
...
def _fallback_entry_key(fallback: dict) -> tuple[str, str, str]:
    return (
        str(fallback.get("provider") or "").strip().lower(),
        str(fallback.get("model") or "").strip(),
        str(fallback.get("base_url") or "").strip().rstrip("/"),
    )


def _fallback_entry_unavailable_without_network(
    agent,
    fallback: dict,
) -> Optional[str]:
    """Return a local skip reason for a fallback that cannot authenticate."""
    fallback_provider = (fallback.get("provider") or "").strip().lower()
    if fallback_provider != "nous":
        return None
    try:
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    if agent._fallback_index >= len(agent._fallback_chain):
        if len(agent._fallback_chain) > 0 and reason not in rate_limited_reasons:
        ...
        if agent.api_mode == "codex_responses":
            ...
            if agent.api_mode == "anthropic_messages":
                ...
                _summary_result = _tsum.normalize_response(
                    summary_response,
                    strip_tool_prefix=(agent._is_anthropic_oauth or _oauth_tool_name_compat),
                )
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions(stream_attempt_id: int):
        ...
        # Google's native Gemini endpoint rejects OpenAI-only stream_options;
        # OpenAI-compatible endpoints use it for final usage accounting.
        ...
        # Some compatibility adapters accept stream=True but return a fully
        # materialized response object. Presence of ``choices`` identifies
        # that shape even when choices is empty/None; do not attempt to iterate
        # it as an SSE stream.
    ...
    if result["error"] is not None:
        if deltas_were_sent["yes"]:
            ...
            # Preserve content-policy classification before swallowing the raw
            # stream error into a continuation stub. The conversation loop can
            # then activate provider fallback immediately instead of retrying a
            # deterministic filter on the same route.
            _content_filter_terminated = False
            try:
                from agent.error_classifier import classify_api_error, FailoverReason
                classification = classify_api_error(
                    result["error"],
                    provider=str(getattr(agent, "provider", "") or ""),
                    model=str(getattr(agent, "model", "") or ""),
                )
                _content_filter_terminated = (
                    classification.reason == FailoverReason.content_policy_blocked
                )
            except Exception:
                _content_filter_terminated = False
