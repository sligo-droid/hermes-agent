...
def _log_compression_degraded_once(
    ...
    *args: Any,
    ...
    """Route repeated compression-health warnings through the aux coalescer."""
    try:
...
def check_compression_model_feasibility(agent: Any) -> None:
    ...
    *after* construction, so :func:`replay_compression_warning` re-sends
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression summaries are unavailable. "
        ...
        resolved_provider = getattr(client, "_hermes_provider", "")
        aux_provider = (
            resolved_provider if isinstance(resolved_provider, str) else ""
        )
        if not aux_provider:
            aux_provider = (
                _aux_cfg_provider
    ...
    if isinstance(content, list):
        ...
        for part in content:
            if isinstance(part, dict):
                total += _content_text_length(
                    part.get("text") or part.get("content") or part.get("data") or ""
                )
            else:
...
def _lineage_row_similar_to_current(
    row: dict[str, Any], current_tokens: int,
) -> bool:
...
def _compression_churn_details(
    ...
    *,
    ...
    current = lineage[0]
    ancestors = lineage[1:]
    lineage_token_baseline = int(
        original_tokens
        or compressed_tokens
        or _lineage_row_input_tokens(current)
        or 0
    )
    current_no_progress = bool(
        ...
        and (
            ...
            or _lineage_row_similar_to_current(
                current, current_tokens=lineage_token_baseline,
            )
        )
    )
    no_progress_lineage = [
        ...
        and _lineage_row_no_progress(
            row, current_tokens=lineage_token_baseline,
        )
    ]
    ...
    zero_message_compression_children = [
        row for row in lineage
        if _lineage_row_message_count(row) == 0 and row.get("parent_session_id")
    ]
    ...
    compressed_over_threshold = bool(
        threshold_tokens and compressed_tokens >= threshold_tokens
    )
    ...
    return {
        ...
        "reasons": reasons,
        "message": (
            "Compression loop detected before rotating to another continuation session."
        ),
    ...
    *,
    ...
    try:
        ...
        if getattr(agent.context_compressor, "_last_compress_aborted", False):
            try:
                ...
                if getattr(agent, "_last_compression_summary_warning", None) != _err:
                    ...
                    _punctuated_err = _err if _err.endswith((".", "!", "?")) else _err + "."
                    agent._emit_warning(
                        f"⚠ Compression aborted: {_punctuated_err} "
        ...
        if not force:
            ...
            if _churn_details:
                emergency_shrink = getattr(
                    agent.context_compressor, "emergency_shrink", None,
                )
                if callable(emergency_shrink):
                    try:
                        ...
                        logger.debug(
                            "compression churn emergency shrink failed",
                            exc_info=True,
                        )
                    else:
                        if _shrunken is not compressed:
                            ...
                            if not _post_shrink_churn:
                                logger.info(
                                    "compression churn avoided by emergency shrink: "
                                    "%s -> %s tokens (stats=%s)",
                                    f"{_compressed_est:,}",
                                    f"{_shrunken_est:,}",
                                    _shrink_stats,
                ...
                if _churn_details:
                    logger.error(
                        "compression churn breaker triggered: %s", _churn_details,
                    )
