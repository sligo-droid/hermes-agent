...
def build_cron_proposal_guidance(
    ...
    guidance = (
        ...
        "Treat coding-worker streams with empty or absent final text as degraded, non-authoritative evidence even when they return structured metadata; an empty coding-worker final must not by itself satisfy a delegated worker evidence floor. "
        "For no-final coding-worker results, inspect top-level `evidence_status` and `failure_class` plus `no_final_metadata.classification`, `no_final_metadata.evidence_status`, `no_final_metadata.failure_class`, `no_final_metadata.local_file_changes`, `no_final_metadata.local_commit_detected`, `no_final_metadata.clean_committed_branch`, `no_final_metadata.branch`, and `no_final_metadata.commit`. "
        "If no-final metadata has no recoverable artifact, describe the stream as degraded or blocked and do not cite it as delegated evidence. "
        "If no-final metadata reports a clean committed artifact, such as `evidence_status: recoverable_degraded` with `local_commit_detected` and `clean_committed_branch`, you may cite only the recoverable artifact metadata as source/static/log evidence; still do not claim the empty final itself is authoritative or satisfies delegated evidence. "
        "Do not reflexively redo parent verification for every degraded no-final worker stream; redo direct verification only when the artifact is small enough and valuable enough to recover within the cron run. "
