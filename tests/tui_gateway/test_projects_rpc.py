...
def test_discover_repos_from_full_history(tmp_path):
    ...
    db = server._get_db()
    prefix = f"projects-{uuid4().hex}"
    session_ids = [f"{prefix}-s{index}" for index in range(1, 4)]
    db.create_session(session_ids[0], "cli", cwd=str(repo))
    db.create_session(session_ids[1], "cli", cwd=str(repo / "src"))
    db.create_session(session_ids[2], "cli", cwd=str(plain))  # not a git repo → excluded
    ...
    assert os.path.realpath(db.get_session(session_ids[0])["git_repo_root"]) == os.path.realpath(str(repo))
