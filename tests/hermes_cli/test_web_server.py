...
class TestWebServerEndpoints:
    ...
    def test_basic_auth_allows_dashboard_requests(self, monkeypatch):
        ...
        unauth_client = TestClient(app)
        basic_headers = {"Authorization": f"Basic {raw}"}

        resp = client.get("/api/status", headers=basic_headers)

        assert resp.status_code == 200
        resp = client.get("/api/dashboard/plugins", headers=basic_headers)
        assert resp.status_code == 200
...
class TestPtyWebSocket:
    ...
    def _setup(self, monkeypatch, _isolate_hermes_home):
        ...
        if not getattr(ws, "_PTY_BRIDGE_AVAILABLE", False):
            pytest.skip("ptyprocess/POSIX PTY bridge is unavailable")
        ...
        monkeypatch.setattr(ws.app.state, "bound_port", None, raising=False)
