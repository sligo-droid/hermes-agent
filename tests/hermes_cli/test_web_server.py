...
def test_fork_dashboard_host_split_and_command_center_routes(monkeypatch):
    """The Sligo ledger stays mounted beside upstream dashboard APIs."""
    ...
    redirect = client.get(
        "/sligo/work?project=hermes",
        headers={"Host": "hermes.sligolabs.com"},
        follow_redirects=False,
    )
    assert redirect.status_code == 307
    assert redirect.headers["location"] == (
        "https://sligo.sligolabs.com/sligo/work?project=hermes"
    )

    paths = [getattr(route, "path", "") for route in ws.app.routes]
    assert "/api/plugins/kanban/command-center/snapshot" in paths
    assert "/api/workers/{session_id}/tickets/{task_id}/console" in paths
    assert "/workers/{session_id}" in paths
    assert paths.index("/api/plugins/kanban/command-center/snapshot") < paths.index(
        "/{full_path:path}"
    )


def test_fork_qa_basic_auth_coexists_with_dashboard_token(monkeypatch):
    from starlette.testclient import TestClient
    import hermes_cli.web_server as ws
    ...
    assert denied.headers["www-authenticate"] == 'Basic realm="Hermes Dashboard"'

    raw = base64.b64encode(b"hermes_qa:qa-password").decode("ascii")
    allowed = client.get("/", headers={"Authorization": f"Basic {raw}"})
    assert allowed.status_code != 401


def test_fork_account_usage_serializer_keeps_weekly_budget_shape():
    ...
    window = SimpleNamespace(
        label="Weekly",
        used_percent=37.5,
        reset_at=datetime(2026, 7, 27, tzinfo=timezone.utc),
        detail="Team allowance",
    )
    snapshot = SimpleNamespace(
        ...
        plan="team",
        windows=[window],
        details=["Current week"],
        unavailable_reason=None,
        fetched_at=datetime(2026, 7, 20, tzinfo=timezone.utc),
    )

    payload = ws._serialize_account_usage_snapshot(snapshot, provider="nous")
    assert payload["available"] is True
    assert payload["plan"] == "team"
    assert payload["windows"][0]["used_percent"] == 37.5
    assert payload["windows"][0]["remaining_percent"] == 62.5
...
class TestWebServerEndpoints:
    ...
    def test_get_config_defaults(self):
        ...
        assert defaults["approvals"]["mode"] == "manual"
