...
migration: POST /api/cron/fire (_find_cron_job_profiles) and
...
def test_cron_fire_profile_lookup_off_loop(monkeypatch, loop_probe):
    ...
    monkeypatch.setattr(web_server, "_find_cron_job_profiles", fake_find)
    monkeypatch.setattr(web_server, "_cron_profile_names", lambda: ["default"])
    monkeypatch.setattr(
        web_server,
        "_verify_cron_fire_token_for_profiles",
        lambda token, profiles: [("default", {"sub": "t"})],
    )

    client = TestClient(web_server.app)
    resp = client.post(
        "/api/cron/fire",
        json={"job_id": "missing-job", "fire_at": "2026-07-18T12:00:00Z"},
        headers={"Authorization": "Bearer x"},
    )
    assert resp.status_code == 200
    assert resp.json()["status"] == "gone"
    assert ("find", False) in seen, (
        f"_find_cron_job_profiles must run off the event loop; proof: {seen}"
    )
