...
async def test_create_cron_job_normalizes_representative_core_fields(
    ...
    # The fork's current reasoning ladder supports max/ultra directly; only
    # older upstream builds collapsed legacy max values to xhigh.
    assert job["reasoning_effort"] == "max"
    assert job["base_url"] == "https://example.invalid/v1"
    # Dashboard cron scripts are stored relative to the profile's scripts/
    # sandbox so the job remains profile-portable and cannot escape that root.
...
def test_cron_job_create_accepts_explicit_null_skills():
    from hermes_cli import web_server

    body = web_server.CronJobCreate(schedule="every 1h", skills=None)
