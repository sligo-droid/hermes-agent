            ...
            "cards": [
                {
                    "id": "card-1",
                    "title": "Cards alias",
                    "summary": "Keep alias ingestion actionable.",
                    "area": "api",
                }
            ],
        }
...
def test_ingest_accepts_marked_json_with_nested_cards(hermes_home):
    marked = """
    Human summary before the machine contract.

    BEGIN_HERMES_SELF_IMPROVEMENT_PROPOSALS
    {"project":"hermes-agent","prong":"quality","cards":[{"id":"marked-1","title":"Marked JSON","summary":"Nested card object parses.","recommended_action":"Keep the marker parser balanced."}]}
    END_HERMES_SELF_IMPROVEMENT_PROPOSALS
    """

    result = sip.ingest_proposals(cron_output_text=marked)

    assert result["parse_status"] == "ok"
    assert result["cards"][0]["id"] == "marked-1"
    assert result["cards"][0]["body"] == "Keep the marker parser balanced."
