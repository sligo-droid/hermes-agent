...
def test_cron_no_proposal_output_records_empty_parsed_run(tmp_path):
    hermes_home = tmp_path / "hermes-home"
    config = {
        "self_improvement": {
            "proposals": {
    ...
    with patch.dict("os.environ", {"HERMES_HOME": str(hermes_home)}):
        result = proposals.ingest_cron_proposal_output({"id": "job-1"}, "All checks healthy. No actionable proposals.", config=config)
        with proposals.connect() as conn:
            run = conn.execute("SELECT * FROM proposal_runs").fetchone()
            card_count = conn.execute("SELECT COUNT(*) FROM proposal_cards").fetchone()[0]
...
def test_cron_proposal_block_scope_mismatch_records_parse_failure(tmp_path):
    hermes_home = tmp_path / "hermes-home"
    config = {
        "self_improvement": {
            "proposals": {
                "projects": {
                    "sligo": {"default_prong": "r1", "prongs": {"r1": {}, "r2": {}}},
                },
                "cron_jobs": {"job-1": {"project": "sligo", "prong": "r1"}},
            }
        }
    ...
    output = "```json\n" + json.dumps(
        {
            proposals.PROPOSAL_BLOCK_KEY: {
                "project": "sligo",
                "prong": "r2",
                "proposals": [{"title": "Wrong scope"}],
            }
        }
    ) + "\n```"

    with patch.dict("os.environ", {"HERMES_HOME": str(hermes_home)}):
        result = proposals.ingest_cron_proposal_output({"id": "job-1"}, output, config=config)
        with proposals.connect() as conn:
            card_count = conn.execute("SELECT COUNT(*) FROM proposal_cards").fetchone()[0]

    assert result["parse_status"] == "failed"
    assert "does not match configured cron scope sligo/r1" in result["parse_error"]
    assert card_count == 0
...
def test_cron_plain_output_without_proposal_markers_records_empty_success(tmp_path):
    hermes_home = tmp_path / "hermes-home"
    config = {
        "self_improvement": {
            "proposals": {
    ...
    with patch.dict("os.environ", {"HERMES_HOME": str(hermes_home)}):
        result = proposals.ingest_cron_proposal_output(
            {"id": "job-1"},
            "Checked the dashboard and logs. No useful self-improvement proposal today.",
            config=config,
        )
        with proposals.connect() as conn:
            run = conn.execute("SELECT * FROM proposal_runs").fetchone()
    ...
    metadata = proposals.parse_json(run["parser_metadata"])
    assert result["parse_status"] == "parsed"
    assert result["card_count"] == 0
    assert run["status"] == "parsed"
    assert metadata["empty_reason"] == "missing_block_no_proposal_markers"
    assert card_count == 0
