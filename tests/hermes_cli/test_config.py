...
class TestVisualQaConfig:
    def test_default_visual_qa_is_shadowed_and_bounded(self):
        assert {
            key: DEFAULT_CONFIG["agent"]["visual_qa"][key]
            for key in ("mode", "max_receipts_per_turn", "max_followup_turns")
        } == {
            "mode": "shadow",
    ...
    def test_visual_qa_partial_override_keeps_bounded_defaults(self, tmp_path):
        ...
        assert {
            key: config["agent"]["visual_qa"][key]
            for key in ("mode", "max_receipts_per_turn", "max_followup_turns")
        } == {
            "mode": "enforce_explicit",
            "max_receipts_per_turn": 1,
...
class TestCustomProviderCompatibility:
    ...
    def test_malformed_legacy_field_does_not_hide_new_style_providers(self, tmp_path):
        config_path = tmp_path / "config.yaml"
        config_path.write_text(
            yaml.safe_dump(
                {
                    "_config_version": 17,
                    "custom_providers": {"unexpected": "mapping"},
                    "providers": {
                        "corp": {
