...
class TestSetupLogging:
    ...
    def test_force_rehomes_handlers_after_hermes_home_changes(self, tmp_path):
        ...
        old_handlers = hermes_logging.rotating_file_handlers()
        ...
        root_handlers = hermes_logging.rotating_file_handlers()
        assert all(
            Path(h.baseFilename).resolve().is_relative_to((new_home / "logs").resolve())
            for h in root_handlers
        )
        assert all(handler.stream is None for handler in old_handlers)

    def test_profile_change_rehomes_handlers_without_force(self, tmp_path):
        old_home = tmp_path / "old-profile"
        new_home = tmp_path / "new-profile"
        marker = "implicit profile rehome marker"

        hermes_logging.setup_logging(hermes_home=old_home)
        old_handlers = hermes_logging.rotating_file_handlers()
        hermes_logging.setup_logging(hermes_home=new_home, mode="gateway")
        logging.getLogger("gateway.run").warning(marker)
        hermes_logging.flush_log_queue()

        assert marker in (new_home / "logs" / "errors.log").read_text()
        assert marker not in (old_home / "logs" / "errors.log").read_text()
        assert all(handler.stream is None for handler in old_handlers)

    def test_shutdown_closes_handlers_before_home_is_removed(self, tmp_path):
        old_home = tmp_path / "removed-profile"
        marker = "must not recreate removed logging home"
        ...
        assert not hermes_logging.rotating_file_handlers()
        assert all(handler.stream is None for handler in old_handlers)

    def test_pytest_isolation_closes_stale_queued_handlers(self, tmp_path):
        old_home = tmp_path / "previous-test"
        next_home = tmp_path / "next-test"

        hermes_logging.setup_logging(hermes_home=old_home, mode="gateway")
        old_handlers = hermes_logging.rotating_file_handlers()
        next_home.mkdir()
        test_conftest._remove_stale_hermes_log_handlers(next_home)
        shutil.rmtree(old_home)

        logging.getLogger("gateway.run").warning("after fixture boundary")
    ...
    def test_pytest_isolation_closes_plain_stale_live_profile_errors_handler(self, hermes_home):
        ...
        assert all(
            ...
            if Path(h.baseFilename).name in {"agent.log", "errors.log", "gateway.log"}
        )
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        ...
        # hermes_plugins facility and bundled platform-plugin modules.
        assert (
            "gateway",
            "hermes_plugins",
            "plugins.platforms",
