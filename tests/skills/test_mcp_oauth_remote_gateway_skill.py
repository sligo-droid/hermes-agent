...
def test_skill_md_uses_progressive_disclosure_with_valid_links():
    content = SKILL_MD.read_text(encoding="utf-8")
    assert len(content) < 20_000

    expected_references = {
        REFERENCES_DIR / "manual-oauth-flow.md",
        REFERENCES_DIR / "stripe-mcp-oauth-revocation.md",
        REFERENCES_DIR / "troubleshooting-and-security.md",
    }
    assert expected_references <= set(REFERENCES_DIR.glob("*.md"))

    assert content.index("### 1. Dashboard first") < content.index(
        "### 2. Native interactive fallbacks"
    ) < content.index("### 3. Manual OAuth bootstrap")

    markdown_files = [SKILL_MD, *sorted(REFERENCES_DIR.glob("*.md"))]
    for source in markdown_files:
        for raw_target in re.findall(r"\[[^]]+\]\(([^)]+)\)", source.read_text()):
            ...
            assert resolved.exists(), f"broken link in {source}: {raw_target}"
