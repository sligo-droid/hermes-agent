...
class TestPrologueStamping:
    def test_stamps_api_content_from_plugin_context(self):
        ...
        assert msg["api_content"].startswith(
            'hello\n\n<automatic-context source="pre_llm_call hook output">'
        )
        assert "PLUGIN-CTX" in msg["api_content"]
    ...
    def test_api_only_override_stamps_sidecar_without_other_injections(self):
        agent = _FakeAgent()
        api_text = "[temporary API guidance]\n\nhello"
        with (
            patch("hermes_cli.goals.agent_goal_context", return_value=""),
        ...
        msg = ctx.messages[ctx.current_turn_user_idx]
        assert msg["api_content"] == api_text
        assert agent.api_content_at_persist == api_text
...
class TestWireInvariant:
    def test_injection_sent_stamped_and_stable_within_turn(self, wire_env):
        ...
        assert sent_1.startswith(
            'hello please\n\n<automatic-context source="pre_llm_call hook output">'
        )
        assert "PLUGIN-CTX" in sent_1
...
class TestPrologueMoaAndInPlaceBackfill:
    ...
    def test_inplace_compaction_backfills_sidecar_into_db(self):
        ...
        with (
            patch(
                "agent.turn_context.automatic_context_budget",
                return_value=1000,
            ),
            patch(
                "hermes_cli.plugins.invoke_hook",
