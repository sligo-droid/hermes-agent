...
class TestNonStringContent:
    ...
    def test_string_message_coerced_to_summary_content(self):
        ...
        assert summary.startswith(
            f"{SUMMARY_PREFIX}\n## Active Task ({HISTORICAL_TASK_HEADING})\n"
        )
...
class TestSummaryFailureTrackingForGatewayWarning:
    ...
    def test_compress_records_fallback_and_dropped_count_on_summary_failure(self):
        ...
        summary_messages = [
            m for m in result
            if isinstance(m.get("content"), str) and SUMMARY_PREFIX in m["content"]
        ]
    ...
    def test_summary_failure_uses_tighter_live_tail(self):
        ...
        assert any(
            isinstance(m.get("content"), str) and SUMMARY_PREFIX in m["content"]
            for m in result
        )
        assert result[-1]["content"] == msgs[-1]["content"]
...
class TestSummaryTargetRatio:
    ...
    def test_default_threshold_is_70_percent(self):
        """At 512K+ the fork's configured default remains 70%."""
        with patch("agent.context_compressor.get_model_context_length", return_value=1_000_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.70
        assert c.threshold_tokens == 700_000

    def test_threshold_floor_does_not_apply_at_or_above_512k(self):
        """On large-context models the configured 70% is used directly."""
        with patch("agent.context_compressor.get_model_context_length", return_value=1_000_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_tokens == 700_000
    ...
    def test_configured_threshold_used_at_512k_and_above(self):
        """At 512K+ the configured fork default is used directly."""
        with patch("agent.context_compressor.get_model_context_length", return_value=512_000):
            c = ContextCompressor(model="test", quiet_mode=True)
        assert c.threshold_percent == 0.70
        assert c.threshold_tokens == 358_400
