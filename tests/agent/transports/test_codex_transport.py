...
class TestCodexBuildKwargs:
    ...
    def test_codex_backend_ephemeral_max_output_tokens(self, transport):
        messages = [{"role": "user", "content": "Hi"}]
        kw = transport.build_kwargs(
            model="gpt-5.4", messages=messages, tools=[],
            max_tokens=4096,
            ephemeral_max_output_tokens=32768,
            is_codex_backend=True,
        )
        assert kw.get("max_output_tokens") == 32768
    ...
    def test_xai_reasoning_effort_passed(self, transport):
        ...
        # The fork does not request opaque encrypted reasoning from xAI:
        # its OAuth Responses surface rejects replayed blobs on later turns.
    ...
    def test_xai_grok_4_omits_reasoning_effort(self, transport):
        ...
        for model in ("grok-4", "grok-4-0709"):
            ...
            # Effort support does not change the xAI replay suppression rule.
            assert kw.get("include") == []
