...
def _write_discord_worker_metadata(board: str, worker: dict[str, object]) -> None:
    from hermes_cli import discord_worker_boards as dwb

    meta = kb.write_board_metadata(board, name=board.replace("-", " ").title())
    meta.pop("db_path", None)
    meta[dwb.DISCORD_WORKER_META_KEY] = {
        "kind": "discord_worker_board",
        "thread_id": board,
...
def test_archive_completed_discord_worker_waits_for_terminal_reaction_sync(client, monkeypatch):
    from hermes_cli import discord_worker_boards as dwb

    board = "discord-archive-unsynced"
    _write_discord_worker_metadata(board, {"goal_status": "done", "phase": "complete"})
    marker_reasons = []
...
def test_archive_completed_discord_worker_stale_cancel_flag_still_waits_for_sync(client, monkeypatch):
    ...
    board = "discord-archive-done-stale-cancelled"
    _write_discord_worker_metadata(board, {"goal_status": "done", "phase": "complete", "cancelled": True})
    marker_reasons = []
    monkeypatch.setattr(dwb, "mark_dispatch_dirty", lambda board, reason: marker_reasons.append((board, reason)))
...
def test_archive_synced_completed_discord_worker_does_not_cancel(client, monkeypatch):
    from hermes_cli import discord_worker_boards as dwb

    board = "discord-archive-synced"
    _write_discord_worker_metadata(board, {"goal_status": "done", "phase": "complete"})
...
def test_archive_cancelled_discord_worker_ignores_stale_reaction_sync(client, monkeypatch):
    from hermes_cli import discord_worker_boards as dwb

    board = "discord-archive-cancelled-stale-sync"
    _write_discord_worker_metadata(
        board,
        {
    ...
    monkeypatch.setattr(
        dwb,
        "stop_board_execution",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("cancelled board should not be stopped")),
    )
    monkeypatch.setattr(
        dwb,
        "mark_dispatch_dirty",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("cancelled board should not wait for sync")),
    )
...
def test_archive_nonterminal_discord_worker_stops_then_waits_for_reaction_sync(client, monkeypatch):
    ...
    board = "discord-archive-running"
    _write_discord_worker_metadata(board, {"goal_status": "active", "phase": "dev"})
    marker_reasons = []
    monkeypatch.setattr(dwb, "mark_dispatch_dirty", lambda board, reason: marker_reasons.append((board, reason)))
    ...
    assert marker_reasons == [
        (board, "terminal-summary-metadata-updated"),
        (board, "archived-from-command-center"),
        (board, "archive-waiting-for-terminal-reaction-sync"),
    ]
...
def test_self_improvement_approve_retry_reattaches_after_record_approval_failure(client, monkeypatch):
    proposal_storage.ingest_proposal_output(_proposal_fixture())
    card = proposal_storage.grouped_cards()["projects"][0]["prongs"][0]["cards"][0]
    publish_calls = []
    created_routes = 0
    ...
    def fake_publish(card_arg, *, channel_id, existing=None):
        nonlocal created_routes
        existing = existing or {}
        publish_calls.append(dict(existing))
        if existing.get("discord_thread_id"):
            return discord_publish.DiscordApprovalRoute(
                channel_id=channel_id,
                top_level_message_id=str(existing["discord_top_level_message_id"]),
                thread_id=str(existing["discord_thread_id"]),
                thread_url=str(existing["discord_thread_url"]),
                board=str(existing["discord_board"]),
    ...
    original_record_approval = proposal_storage.record_approval
    failures = {"remaining": 1}

    def flaky_record_approval(*args, **kwargs):
        if failures["remaining"]:
            failures["remaining"] -= 1
            raise RuntimeError("storage unavailable")
        return original_record_approval(*args, **kwargs)
    ...
    assert second.status_code == 200, second.text
    assert created_routes == 1
    assert publish_calls[0] == {}
    assert publish_calls[1]["discord_thread_id"] == "777"
    assert publish_calls[1]["discord_top_level_message_id"] == "555"
    assert second.json()["worker_url"].endswith("/workers/discord-777-m-555")
    ...
    assert actions[:2] == ["approval_route_created", "approval_record_failed"]
    assert audit[1]["kanban_task_id"] == second.json()["task"]["id"]
    assert audit[1]["metadata"]["worker_url"].endswith("/workers/discord-777-m-555")
...
def test_self_improvement_approve_short_circuits_already_approved_card(client, monkeypatch):
    ...
    assert second.json()["worker_url"] == first.json()["worker_url"]
    conn = kb.connect()
    try:
        assert conn.execute("SELECT COUNT(*) FROM tasks").fetchone()[0] == 1
    finally:
...
def test_self_improvement_approve_record_approval_failure_records_retry_metadata(client, monkeypatch):
    proposal_storage.ingest_proposal_output(_proposal_fixture())
    card = proposal_storage.grouped_cards()["projects"][0]["prongs"][0]["cards"][0]
    ...
    audit = proposal_storage.list_audit_events(card["proposal_id"])
    assert audit[-1]["action"] == "approval_record_failed"
    assert audit[-1]["kanban_task_id"]
    assert audit[-1]["reason"] == "storage unavailable"
    assert audit[-1]["metadata"]["board"] == "discord-777-m-555"
    assert audit[-1]["metadata"]["discord_top_level_message_id"] == "555"
    assert audit[-1]["metadata"]["discord_thread_id"] == "777"
    assert audit[-1]["metadata"]["worker_url"].endswith("/workers/discord-777-m-555")
