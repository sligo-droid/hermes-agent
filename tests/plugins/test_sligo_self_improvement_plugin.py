...
def test_ingest_rejects_missing_body_and_card_floods(hermes_home):
    missing_body = _payload("Missing body")
    missing_body["cards"][0].pop("recommended_action")
    missing_body["cards"][0].pop("worker_prompt")
    missing_body["cards"][0].pop("summary")

    with pytest.raises(proposals.ProposalError, match="body is required"):
        proposals.ingest_proposals(proposal_json=missing_body)

    too_many = _payload("Too many")
    too_many["cards"] = [
        dict(too_many["cards"][0], title=f"Card {i}")
        for i in range(proposals.MAX_CARDS_PER_RUN + 1)
    ]
    with pytest.raises(proposals.ProposalError, match="too many cards"):
        proposals.ingest_proposals(proposal_json=too_many)
