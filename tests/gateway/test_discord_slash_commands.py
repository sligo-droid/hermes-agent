...
def test_build_slash_event_preserves_thread_context(adapter):
    ...
    assert event.native_slash_command is True
    assert event.source.chat_id == "555"
    assert event.source.chat_type == "thread"
    assert event.source.thread_id == "555"
    assert "TestGuild" in event.source.chat_name
...
@pytest.mark.asyncio
async def test_auto_create_thread_uses_seed_message_fallback(adapter):
    fallback_thread = SimpleNamespace(id=555, name="Hello")
    ...
    result = await adapter._auto_create_thread(message)
    assert result is fallback_thread
    message.create_thread.assert_awaited_once()
...
@pytest.mark.asyncio
async def test_auto_create_thread_returns_none_when_seed_fallback_fails(adapter):
    message = SimpleNamespace(
        content="Hello",
        create_thread=AsyncMock(side_effect=RuntimeError("no perms")),
...
@pytest.mark.parametrize(
    ...
    [
        ("/fable plan the work", "Fable plan — plan the work"),
        ("/fable help me plan to ship the work", "Fable plan — help me plan to ship the work"),
    ],
)
async def test_fable_plan_slash_in_parent_channel_creates_thread_before_dispatch(
    adapter,
    command_text,
    thread_name,
):
...
@pytest.mark.asyncio
async def test_fable_implementation_slash_uses_normal_non_kanban_action_thread(adapter):
    parent = _FakeTextChannel(channel_id=123, name="planning")
    thread = _FakeThreadChannel(channel_id=555, name="Fable — build the work", parent_id=123)
    ...
    adapter._create_thread = AsyncMock(
        return_value={
            "success": True,
            "thread_id": "555",
            "thread_name": "Fable — build the work",
            "_thread": thread,
        }
    )
    ...
    adapter.initialize_feature_summary = AsyncMock(
        return_value={"initial_request": "/fable build the work", "kanban_board": None}
    )
    captured_events = []
    adapter.handle_message = AsyncMock(side_effect=lambda event: captured_events.append(event))

    await adapter._run_simple_slash(interaction, "/fable build the work")

    adapter._create_thread.assert_awaited_once_with(
        interaction,
        name="Fable — build the work",
        message="",
        auto_archive_duration=1440,
        reason_command="fable",
    )
...
@pytest.mark.asyncio
async def test_goal_slash_without_args_routes_to_hermes_goal_loop(adapter):
    interaction = SimpleNamespace()
    adapter._run_simple_slash = AsyncMock()
