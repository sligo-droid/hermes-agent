...
async def test_discord_action_request_keeps_full_platform_tool_surface(monkeypatch):
    ...
    assert "must never run mutable terminal commands" in init["ephemeral_system_prompt"]
    assert "exclusively owns exact-SHA canonical synchronization" in init["ephemeral_system_prompt"]
    assert "Do not proactively call sync_canonical_checkout" in init["ephemeral_system_prompt"]
    assert "cleanup from the mutable action worktree" in init["ephemeral_system_prompt"]
    assert "Do not report canonical sync as failed or pending" in init["ephemeral_system_prompt"]
