...
@pytest.mark.asyncio
async def test_prepare_inbound_message_text_labels_discord_voice_as_sender_note():
    from gateway.run import GatewayRunner
    ...
    with patch(
        ...
        return_value={
            "success": True,
            "transcript": "remember to update the client summary",
            "provider": "local_command",
        },
    ):
        result = await runner._prepare_inbound_message_text(
    ...
    assert result is not None
    assert "remember to update the client summary" in result
    assert "sender/intermediary" in result
    assert "not as a client document" in result
