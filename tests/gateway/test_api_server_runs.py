...
class TestStartRun:
    ...
    @pytest.mark.asyncio
    async def test_start_uses_configured_shared_concurrency_cap(self, adapter):
    ...
    @pytest.mark.asyncio
    async def test_start_passes_model_route_to_agent(self):
        route = {"model": "minimax/minimax-m1", "provider": "openrouter"}
        config = PlatformConfig(
            enabled=True,
            extra={"model_routes": {"minimax-m2": route}},
        )
        adapter = APIServerAdapter(config)
        app = _create_runs_app(adapter)
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_create_agent") as mock_create:
                ...
                resp = await cli.post(
                    "/v1/runs",
                    json={"model": "minimax-m2", "input": "hello"},
                )
                assert resp.status == 202
                ...
                assert mock_create.call_args.kwargs["route"] == route
