...
def test_adapter_for_source_missing_secondary_fails_closed(monkeypatch):
    """A stamped unknown secondary profile must never borrow the primary bot."""
    runner, default_adapter, _secondary_adapter = _make_multiplex_runner(monkeypatch)
...
def test_public_runner_uses_authoritative_profile_authz_mixin():
    """The final wrapper MRO must beat stale methods on the merged core class."""
    from gateway.authz_mixin import GatewayAuthorizationMixin
    from gateway.run import GatewayRunner

    assert GatewayRunner._adapter_for_source is GatewayAuthorizationMixin._adapter_for_source
    assert (
        GatewayRunner._get_unauthorized_dm_behavior
        is GatewayAuthorizationMixin._get_unauthorized_dm_behavior
    )
