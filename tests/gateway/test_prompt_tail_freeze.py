...
class TestSessionContextPin:
    def test_live_gateway_turn_uses_pinned_renderer(self):
        from gateway.run import GatewayRunner

        live_names = set(
            GatewayRunner._handle_message_with_agent.__code__.co_names  # noqa: SLF001
        )
        assert "_pinned_session_context_prompt" in live_names
        assert "build_session_context_prompt" not in live_names
