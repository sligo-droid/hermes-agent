...
def _canonical_item(tmp_path):
    item = _item(tmp_path)
    item["closeout"]["workspace"]["canonical_path"] = str(tmp_path / "canonical")
    item["closeout"]["policy"]["post_merge_requirements"] = {
        "canonical_sync": True
    }
    return item
...
def test_observer_hands_required_canonical_sync_to_trusted_closeout(tmp_path):
    payload = {
        "number": 1035,
        "url": "https://github.com/sligo-labs/PID/pull/1035",
        "state": "MERGED",
    ...
    state = observe_merged_direct_closeout(
        _canonical_item(tmp_path),
        "Done: https://github.com/sligo-labs/PID/pull/1035",
        run=run,
    )

    assert state["status"] == "post_merge_pending"
    assert state["next_due_at"] == 0.0
    assert state["post_merge"]["target_sha"] == MERGE
    assert state["post_merge"]["canonical_sync"] == {"status": "pending"}
...
def test_ledger_adopts_exact_pending_post_merge_observation(tmp_path):
    ledger = GatewayWorkLedger(tmp_path / "ledger.json", now_fn=lambda: 50.0)
    data = {
        "version": 1,
        "items": {
            "work": {
                ...
                "status": "agent_running",
                "closeout_authoritative": False,
                "closeout": _canonical_item(tmp_path)["closeout"],
            }
        },
    }
    ledger.path.write_text(json.dumps(data), encoding="utf-8")
    observed = _canonical_item(tmp_path)["closeout"]
    observed.update(
        {
    ...
    assert stored["closeout_authoritative"] is True
    assert stored["closeout"]["status"] == "post_merge_pending"
    assert stored["closeout"]["post_merge"]["target_sha"] == MERGE
