...
def test_config_get_approval_mode_preserves_configured_smart(tmp_path, monkeypatch):
    import yaml
...
def test_config_set_approval_mode_persists_three_way_value_and_emits_live_status(
    ...
    assert resp["result"] == {"key": "approvals.mode", "value": mode}
    assert yaml.safe_load((tmp_path / "config.yaml").read_text())["approvals"]["mode"] == mode
    assert emitted and emitted[0][0:2] == ("session.info", "sid")
    assert emitted[0][2]["approval_mode"] == mode
...
def test_session_most_recent_skips_empty_gateway_rows(monkeypatch):
    class _DB:
    ...
    assert resp["result"]["session_id"] == "real-tui"


def test_session_list_omits_empty_gateway_rows(monkeypatch):
    class _DB:
        def list_sessions_rich(self, **kwargs):
            return [
                {"id": "empty", "source": "slack", "message_count": 0},
                {"id": "real", "source": "tui", "message_count": 1},
            ]
    ...
    assert [row["id"] for row in resp["result"]["sessions"]] == ["real"]
