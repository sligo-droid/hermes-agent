...
def test_run_one_job_records_running_then_terminal(monkeypatch, tmp_path):
    import cron.scheduler as scheduler
    ...
    monkeypatch.setattr(
        scheduler, "save_job_output", lambda *_args: tmp_path / "job-3.md"
    )
    monkeypatch.setattr(scheduler, "update_job_output", lambda *_args: None)
