...
class TestResolveGatewayModel:
    """Test legacy model-key fallback when gateway tier routing is disabled."""

    def test_returns_default_key(self):
        from gateway.run import _resolve_gateway_model
        config = {
            "gateway": {"model_tier": "disabled"},
            "model": {"default": "gpt-5.4"},
        }
        assert _resolve_gateway_model(config) == "gpt-5.4"
    ...
    def test_returns_empty_when_missing(self):
        from gateway.run import _resolve_gateway_model
        config = {"gateway": {"model_tier": "disabled"}, "model": {}}
        assert _resolve_gateway_model(config) == ""

    def test_returns_empty_when_no_model_section(self):
        from gateway.run import _resolve_gateway_model
        config = {"gateway": {"model_tier": "disabled"}}
        assert _resolve_gateway_model(config) == ""
    ...
    def test_string_model_config(self):
        ...
        assert _resolve_gateway_model(config) == "my-model"
