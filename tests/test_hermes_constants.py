...
class TestIsContainer:
    ...
    def test_detects_kubernetes_env(self, monkeypatch):
        """KUBERNETES_SERVICE_HOST triggers detection for Kubernetes pods."""
        self._reset_cache(monkeypatch)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
    ...
    def test_detects_cgroup_kubepods(self, monkeypatch, tmp_path):
        """A kubepods cgroup marker triggers container detection."""
        import builtins
        self._reset_cache(monkeypatch)
        monkeypatch.delenv("KUBERNETES_SERVICE_HOST", raising=False)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
    ...
    def test_detects_cgroup_v2_via_mountinfo(self, monkeypatch, tmp_path):
        """cgroup v2 falls back to the runtime marker in mountinfo."""
        import builtins
        self._reset_cache(monkeypatch)
        monkeypatch.delenv("KUBERNETES_SERVICE_HOST", raising=False)
        monkeypatch.setattr(os.path, "exists", lambda p: False)
