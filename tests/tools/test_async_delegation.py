...
def test_advisory_delegation_uses_generic_durable_store(monkeypatch, tmp_path):
    ...
    claim_id = ad.claim_event_delivery(event, "test-advisory")
    assert claim_id
...
def test_delegate_task_background_detaches_child_from_parent(monkeypatch):
    ...
    gate.set()
    event = _drain_one()
    assert event is not None, ad.list_async_delegations()
...
def test_capacity_is_shared_between_coding_workers_and_delegations():
    gate = threading.Event()
    ...
    assert ad.active_count(kind="delegation") == 0
    gate.set()
    assert _drain_one() is not None

    delegated_after = ad.dispatch_async_delegation_batch(
        goals=["delegated"],
        context=None,
        toolsets=None,
        role="leaf",
    ...
    assert delegated_after["status"] == "dispatched"
    assert _drain_one() is not None
