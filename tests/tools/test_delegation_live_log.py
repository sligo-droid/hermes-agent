...
def test_real_child_execution_path_streams_assistant_text_to_live_log(monkeypatch):
    """delegate_task -> _run_single_child wires the real text callback."""
    import tools.delegate_tool as dt
    ...
    class StreamingChild:
        ...
        def run_conversation(self, *, user_message, task_id, stream_callback=None):
            assert user_message == "streamed prose goal"
            assert task_id.startswith("subagent-0-")
            assert callable(stream_callback)
            stream_callback("Real child assistant ")
            stream_callback("prose arrives incrementally.")
            return {
                "final_response": "Real child assistant prose arrives incrementally.",
                "completed": True,
                "api_calls": 1,
                "messages": [],
            }
    ...
    out = json.loads(dt.delegate_task(
        goal="streamed prose goal",
        parent_agent=parent,
    ))

    text = Path(out["live_transcripts"][0]).read_text(encoding="utf-8")
    assert "Real child assistant prose arrives incrementally." in text
