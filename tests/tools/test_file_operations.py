...
class _DeletedTestGitBaselineCheck:
    """Do not collect stale tests for the nonexistent git-baseline helper.

    Upstream removed this contract because ``ShellFileOperations`` has no
    ``_check_git_baseline`` method.  Keep the historical cases inert until
    that helper is deliberately restored or replaced; implementing it solely
    to satisfy these stale assertions would add unrequested write behavior.
    """
