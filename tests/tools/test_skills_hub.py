...
# ---------------------------------------------------------------------------
# Profile-scoped dynamic hub paths
# ---------------------------------------------------------------------------


def test_restored_dynamic_skills_dir_does_not_pin_previous_profile(
    monkeypatch, tmp_path
):
    """A patch teardown must not freeze a prior profile's dynamic path."""
    import tools.skills_hub as hub

    previous_home = tmp_path / "previous"
    active_home = tmp_path / "active"
    monkeypatch.setenv("HERMES_HOME", str(previous_home))
    hub.__dict__.pop("SKILLS_DIR", None)

    # Simulate a patch helper reading __getattr__ and later restoring the
    # concrete fallback Path as a real module attribute.
    restored_value = getattr(hub, "SKILLS_DIR")
    setattr(hub, "SKILLS_DIR", restored_value)

    monkeypatch.setenv("HERMES_HOME", str(active_home))
    assert hub._skills_dir() == active_home / "skills"
    assert "SKILLS_DIR" not in hub.__dict__
