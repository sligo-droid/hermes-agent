...
def agent():
    with (
        ...
        a.tool_delay = 0
        # Overflow-recovery tests exercise the production default. Dedicated
        # disabled-path tests below set this back to False explicitly.
...
class TestPreflightCompression:
    ...
    def test_preflight_compresses_oversized_history(self, agent):
        ...
        # that the compressed result plus the fork's runtime prompt/tool
        # overhead fits in a single pass.
        agent.context_compressor.context_length = 2000
        agent.context_compressor.threshold_tokens = 600
        agent.context_compressor.protect_last_n = 10

        # Build a history that will be large enough to trigger preflight
        # (40 messages are ~800 tokens here, above the 600-token threshold).
        big_history = []
        for i in range(20):
