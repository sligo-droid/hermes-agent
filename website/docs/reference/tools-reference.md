...
| `web_extract` | Extract clean content from up to 5 web page or PDF URLs without LLM summarization. Long pages return a deterministic head/tail view; omitted middle text is not stored. Per-page limits are 2,000–90,000 characters, with a 90,000-character aggregate content budget per call. | EXA_API_KEY or PARALLEL_API_KEY or FIRECRAWL_API_KEY or TAVILY_API_KEY |
