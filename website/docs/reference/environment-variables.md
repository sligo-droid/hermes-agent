...
| `MISTRAL_API_KEY` | Mistral API key for Voxtral TTS/STT. Currently unused because Hermes has temporarily disabled Voxtral while `mistralai` is quarantined. |
...
| `DISCORD_THREAD_REQUIRE_MENTION` | Require an @mention before responding in Discord threads (default: `true`) |
| `DISCORD_VOICE_AUTO_TAG` | Treat native Discord voice-message posts as addressed to the bot even without @mention (default: `false`) |
...
| `HERMES_GATEWAY_BUSY_INPUT_MODE` | Gateway busy-input behavior: `steer` (default), `queue`, or `interrupt`. Can be overridden per chat with `/busy`. |
| `HERMES_GATEWAY_BUSY_STEER_ACK_ENABLED` | Whether accepted live steers receive a visible next-safe-boundary acknowledgement (default: `true`). Set `false` to keep steering silent. |
...
| Variable | Description |
|----------|-------------|
| `HERMES_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 1000) |
