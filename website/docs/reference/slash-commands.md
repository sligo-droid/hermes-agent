...
| `/sessions` | Browse and resume previous sessions in an interactive CLI picker. |
...
| `/indicator [kaomoji\|emoji\|unicode\|ascii]` | CLI-only: pick the busy-indicator style. |
...
- Use `/resume [name]` or `hermes --resume <id-or-title>` for saved or closed transcripts.
