...
| `/stop` | Stop all work owned by the current gateway session |
| `/queue <prompt>` (alias: `/q`) | Queue a prompt for the next turn (doesn't interrupt the current agent response). |
| `/steer <prompt>` | Guide the current run at its next safe boundary. Already-started work finishes; later unstarted calls are skipped before the model replans. If the turn is closed or unsupported, the prompt is queued for the immediate next turn. |
...
| `/skills ...` | Search, browse, inspect, vet, install, audit, publish, and configure skills from registries and the official optional-skills catalog. |
...
| `/stop` | Interrupt the active agent and synchronous children, cancel session-owned detached delegations and `/background` agents, kill tracked session terminal processes, and stop matching Discord worker boards. Undelivered canceled completions are suppressed. Other sessions, profiles, cron jobs, and unrelated Kanban work are unaffected. |
...
| `/steer <prompt>` | Guide the current run at its next safe boundary without cancelling already-started work; closed or unsupported turns queue it immediately. |
...
- `/skin`, `/snapshot`, `/reload`, `/tools`, `/toolsets`, `/browser`, `/config`, `/cron`, `/skills`, `/platforms`, `/paste`, `/image`, `/statusbar`, `/plugins`, `/busy`, `/indicator`, `/redraw`, `/clear`, `/history`, `/save`, `/copy`, `/handoff`, and `/quit` are **CLI-only** commands.
