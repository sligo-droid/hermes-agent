---
title: "Thermo Nuclear Code Quality Review — Thermonuclear deep code quality maintainability review"
sidebar_label: "Thermo Nuclear Code Quality Review"
description: "Thermonuclear deep code quality maintainability review"
---
...
Thermonuclear deep code quality maintainability review.
...
| Path | `skills/software-development/thermo-nuclear-code-quality-review` |
| Version | `1.0.0` |
| Author | Cursor team, adapted by Hermes Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `code-review`, `maintainability`, `refactor`, `architecture`, `quality` |
| Related skills | [`simplify-code`](/docs/user-guide/skills/bundled/software-development/software-development-simplify-code) |
...
maintainability, abstraction quality, and codebase health. Invoke it only for
explicit requests such as "thermo-nuclear review", "thermonuclear review",
"deep code quality audit", or "harsh maintainability review".

Source note: adapted from Cursor's `thermo-nuclear-code-quality-review` skill in
`cursor-team-kit/skills/thermo-nuclear-code-quality-review/SKILL.md`.
Cursor-only metadata such as `disable-model-invocation` is intentionally omitted.
...
- The user explicitly asks for a thermo-nuclear, thermonuclear, deep code
  quality, or harsh maintainability review.
- A review request asks specifically for ambitious structural simplification,
  abstraction quality, or maintainability pressure rather than ordinary
  correctness/security review.
- The requested review should challenge whether the implementation belongs in
  its current shape, layer, file, or abstraction.

Do not use this skill automatically for generic "review", "pre-commit review",
"verify", "ship", or "code review" requests. Generic code review stays in the
normal review skills unless the user explicitly asks for this stricter mode.
...
0. **Be ambitious about structural simplification.**
   ...
   - Look for opportunities to reframe the change so whole branches, helpers,
     modes, conditionals, or layers disappear entirely.
   - Prefer the solution that makes the code feel inevitable in hindsight.
   - Assume there is often a "code judo" move available: a reorganization that
...
1. **Do not let a PR push a file from under 1k lines to over 1k lines without a
   very strong reason.**
...
- Bespoke helpers where the codebase already has a canonical utility for the
  job.
- Logic added in the wrong layer/package when it should live somewhere more
  central.
...
- `why does this need a cast / optional here? can we make the boundary more
  explicit instead?`
- `this looks like a bespoke helper for something we already have elsewhere. can
  we reuse the canonical one?`
- `i think there's a code-judo move here that makes this much simpler. can we
  reframe this so these branches disappear?`
- `this refactor moves complexity around, but doesn't really delete it. is there
  a way to make the model itself simpler?`
...
- The PR duplicates an existing helper or puts logic in the wrong layer when
...
- Confirm the request explicitly asked for thermo-nuclear, thermonuclear, deep
  code quality, or harsh maintainability review before applying this stricter
  mode.
- Scope the review to the requested diff, branch, files, or PR.
- Ground each blocker in a concrete file, line, or pattern and propose the
  smallest structural remedy that would actually reduce complexity.
- Separate true structural blockers from optional cleanup notes.
