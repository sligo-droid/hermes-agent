...
Fires **immediately before** an approval request is shown to the user — covers every surface: interactive CLI, gateway platforms (Telegram, Discord, Slack, WhatsApp, Matrix, etc.), and ACP clients (VS Code, Zed, JetBrains).
...
| `surface` | `str` | `"cli"` for interactive CLI prompts, `"gateway"` for async platform approvals |
