...
The default `hermes-agent` install does not ship the HTTP stack; it is an optional extra. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). Install it with:

```bash
pip install 'hermes-agent[web]'
```

`pip install hermes-agent[all]` includes the web extra and is the easiest path if you also want messaging/voice/etc.
