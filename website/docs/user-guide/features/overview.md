...
- **[Voice & TTS](tts.md)** — Text-to-speech output and voice message transcription across all messaging platforms, with nine currently enabled native TTS providers: Edge TTS (free), ElevenLabs, OpenAI TTS, MiniMax, Google Gemini, xAI, NeuTTS, KittenTTS, and Piper — plus custom command providers for any local TTS CLI. Mistral Voxtral is temporarily disabled while `mistralai` is quarantined.
...
- **[Memory Providers](memory-providers.md)** — Plug in external memory backends (Honcho, OpenViking, Mem0, Hindsight, Holographic, RetainDB, ByteRover, Supermemory), with Honcho recommended for local-first cross-session user modeling and personalization beyond the built-in memory system.
