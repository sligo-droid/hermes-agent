...
Delegation is synchronous by default. A model may explicitly request `background=true` for read-only work on a session that can accept a later completion turn; Hermes then returns a handle and posts the result back as a new message. Orchestrator subagents wait for their own workers so they can synthesize the results before returning.
...
`deep_review` is a reserved Sol/`high` special case. It is available only on
...
When an agent provides a `tasks` array, Hermes runs the subagents in parallel and returns one consolidated result after every child finishes. If the caller explicitly requests `background=true` and the session supports later delivery, Hermes returns one background handle instead. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
