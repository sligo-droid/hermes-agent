...
| **AGENTS.md** | Project instructions, conventions, architecture | CWD startup first-match project context; subdirectories as lazy hints |
| **CLAUDE.md** | Claude Code context files (also detected) | CWD startup first-match project context; subdirectories as lazy hints |
...
At session start, Hermes scans the current working directory using the project-context priority order (`.hermes.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`) and loads the first matching project context type into the system prompt. If `AGENTS.md` wins that startup priority, that CWD file becomes the startup project instruction file. As the agent navigates into subdirectories during the session (via `read_file`, `terminal`, `search_files`, etc.), Hermes may surface nested context files as **lazy tool-result hints**. Those hints help with local relevance, but they are not merged into the startup project context.

```
my-project/
├── AGENTS.md              ← Loaded at startup if AGENTS.md wins priority
├── frontend/
│   └── AGENTS.md          ← Surfaced as a tool-result hint for frontend/ files
├── backend/
│   └── AGENTS.md          ← Surfaced as a tool-result hint for backend/ files
└── shared/
    └── AGENTS.md          ← Surfaced as a tool-result hint for shared/ files
```

This approach has two advantages over loading everything at startup:
- **No system prompt bloat** — subdirectory hints only appear in relevant tool results
- **Prompt cache preservation** — the system prompt stays stable across turns

Each subdirectory is checked at most once per session. The hint discovery also walks up parent directories, so reading `backend/src/main.py` can surface `backend/AGENTS.md` even if `backend/src/` has no context file of its own.
...
1. **Scan working directory** — checks for `.hermes.md` / `HERMES.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules` (first match wins)
...
### During the session (lazy subdirectory hints)
...
3. **Hint lookup** — if an `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` is found, a bounded hint is prepared (first match per directory)
4. **Security scan** — same prompt injection scan as startup files
5. **Truncation** — capped at 8,000 characters per file
6. **Tool-result hint** — appended to the relevant tool result, so the model can use it locally without changing startup project context
