...
compression:
  ...
  abort_on_summary_failure: false # Use degraded local fallback if summary fails
...
| `abort_on_summary_failure` | `false` | boolean | If true, preserve the original messages unchanged when summarization fails; false inserts a deterministic fallback handoff and drops the middle window |
...
GitHub Copilot). At the default 70% trigger, compaction would fire at ~190K.
When the active route is Codex
