...
| `web_extract` | 从最多 5 个网页或 PDF URL 提取清理后的内容，不调用 LLM 摘要。长页面返回确定性的头尾视图；省略的中间文本不会存储。单页范围为 2,000–90,000 个字符，每次调用共享 90,000 个字符的内容总预算。 | EXA_API_KEY 或 PARALLEL_API_KEY 或 FIRECRAWL_API_KEY 或 TAVILY_API_KEY |
