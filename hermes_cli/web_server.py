...
_TESTCLIENT_HOST_VALUES: frozenset = frozenset({"testclient", "testserver"})
...
def _is_loopback_host(value: str, *, allow_testclient: bool = False) -> bool:
    ...
    if normalized in _LOOPBACK_HOST_VALUES or (
        allow_testclient and normalized in _TESTCLIENT_HOST_VALUES
    ):
        return True
    try:
...
def _status_request_is_direct_loopback(request: Request) -> bool:
    ...
    Forwarding and edge headers are redaction signals, never evidence that a
    request is local, so spoofing them can only reduce the response.
    """
    if getattr(request.app.state, "auth_required", False):
        return False
    ...
    return _is_loopback_host(
        direct_client, allow_testclient=True
    ) and _is_loopback_host(
        request.headers.get("host", ""), allow_testclient=True
    )
...
async def auth_middleware(request: Request, call_next):
    """Enforce dashboard HTTP auth while preserving public bootstrap routes."""
    ...
    if getattr(request.state, "token_authenticated", False):
        return await call_next(request)
    path = request.url.path
    # Dashboard plugin browser assets are public by design: script/link tags
    # cannot attach the session header or HTTP Basic credentials. The route
    # itself restricts user plugins to enabled entries and serves only a
    # browser-asset suffix allowlist.
    if path.startswith("/dashboard-plugins/"):
        return await call_next(request)
    # When the OAuth gate is active, cookie-based auth (gated_auth_middleware
    # above) is authoritative unless the explicitly configured legacy Basic
    # path is the only registered gate. Valid Basic credentials use the same
    # request-state seam as service tokens so downstream handlers accept them.
    if getattr(request.app.state, "auth_required", False):
        if _legacy_basic_auth_enabled():
            if _has_valid_basic_auth(request):
                request.state.token_authenticated = True
                request.state.legacy_basic_authenticated = True
                return await call_next(request)
            try:
                from hermes_cli.dashboard_auth import list_providers

                if not list_providers() and path not in _PUBLIC_API_PATHS:
                    return _basic_auth_challenge()
            except Exception:
                if path not in _PUBLIC_API_PATHS:
                    return _basic_auth_challenge()
        return await call_next(request)
    ...
    if path not in _PUBLIC_API_PATHS and _legacy_basic_auth_required(request):
        if _has_valid_session_token(request) or _has_valid_query_token(request, path):
            return await call_next(request)
        if _has_valid_basic_auth(request):
            request.state.token_authenticated = True
            request.state.legacy_basic_authenticated = True
...
async def get_status(request: Request, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            pass
        if _legacy_basic_auth_enabled() and "legacy-basic" not in auth_providers:
            auth_providers.append("legacy-basic")
        ...
        # caller is already inside the trust envelope. Forwarding/edge headers
        # and a non-loopback peer always redact, even on an insecure bind.
...
def _messaging_platform_payload(
    ...
    if scoped:
        ...
        try:
            ...
            # Environment-derived gateway compatibility defaults must not
            # override an explicit dashboard/config choice. In particular, a
            # stored token can make GatewayConfig infer ``enabled=True`` even
            # immediately after the operator switched the platform off.
            raw_platforms = (load_config().get("platforms") or {})
            raw_platform = raw_platforms.get(platform_id)
            if isinstance(raw_platform, dict) and "enabled" in raw_platform:
                enabled = bool(raw_platform.get("enabled"))
        except Exception:
            enabled = False
            configured = all(
...
def _normalize_dashboard_cron_updates(
    ...
    for key in ("model_tier", "model", "provider", "reasoning_effort", "workdir"):
...
def _cron_profile_names() -> List[str]:
    """Return configured profile names once, without empty duplicates."""
    names: List[str] = []
...
def _create_cron_job_sync(body: CronJobCreate, profile: Optional[str] = None):
    try:
        ...
        return _call_cron_for_profile(
            ...
            skills=skills,
            model_tier=_cron_optional_text(body.model_tier),
...
async def cron_fire_webhook(request: Request):
    """Accept a provider fire after verifying its profile-scoped credential."""
    auth = request.headers.get("Authorization", "")
    ...
    job_profiles = (
        await _run_cron_dashboard_io(_find_cron_job_profiles, job_id)
        if job_id
        else []
    )
    profiles_to_verify = job_profiles or await _run_cron_dashboard_io(
        _cron_profile_names
    )
    verified_profiles = await _run_cron_dashboard_io(
    ...
    if not await _run_cron_dashboard_io(
    ...
    try:
        process, token_key, fire_key = await _run_cron_dashboard_io(
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    issues from the log.
    """
    # The private fork's QA/operator automation authenticates the dashboard
    # with HTTP Basic credentials. Browsers carry that Authorization header on
    # the same-origin WebSocket upgrade, so accept it through the same narrow
    # compatibility seam as HTTP requests without weakening ticket/token auth.
    if _legacy_basic_auth_enabled() and _has_valid_basic_auth(ws):
        return None, "basic"
...
async def _resolve_chat_argv_async(
    ...
    if active_session_file is not None:
        # Keep the seam monkeypatchable by older tests/plugins whose resolver
        # predates PTY reattach. The shipped resolver accepts this argument;
        # compatibility callables without it still get the new async/off-loop
        # launch path, just without the breadcrumb file.
        try:
            resolver_params = inspect.signature(_resolve_chat_argv).parameters
        except (TypeError, ValueError):
            resolver_params = {}
        if "active_session_file" in resolver_params or any(
            param.kind is inspect.Parameter.VAR_KEYWORD
            for param in resolver_params.values()
        ):
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = "", surface: str = "combined"):
        ...
        product shell so the same bundle can preserve the Hermes/Sligo split.
...
# ---------------------------------------------------------------------------
# Fork dashboard compatibility surfaces
# ---------------------------------------------------------------------------
...
def _dashboard_basic_password() -> str:
    return os.getenv("HERMES_DASHBOARD_PASSWORD", "")


def _legacy_basic_auth_enabled() -> bool:
    return bool(_dashboard_basic_password()) or _env_flag_enabled(
        "HERMES_DASHBOARD_REQUIRE_BASIC_AUTH"
    )


def _legacy_basic_auth_required(request: Request) -> bool:
    """Fail closed outside a confirmed loopback bind when OAuth is inactive."""
    if _legacy_basic_auth_enabled():
        return True
    bound_host = str(getattr(request.app.state, "bound_host", "") or "").strip().lower()
    return not bound_host or bound_host not in _LOOPBACK_HOST_VALUES
...
def _has_valid_basic_auth(request: Any) -> bool:
    auth = request.headers.get("authorization", "")
    ...
    if not sep:
        return False
    expected_password = _dashboard_basic_password()
    return bool(expected_password) and _compare_secret(
        username, _dashboard_basic_username()
    ) and _compare_secret(password, expected_password)
...
def dashboard_surface_for_host(host: str) -> str:
    """Classify the public dashboard host without affecting local/custom binds."""
    normalized = _dashboard_host_without_port(host)
    hermes_host = os.getenv(
        "HERMES_DASHBOARD_HOST", _DEFAULT_HERMES_DASHBOARD_HOST
    ).lower()
    sligo_host = os.getenv(
        "SLIGO_DASHBOARD_HOST", _DEFAULT_SLIGO_DASHBOARD_HOST
    ).lower()
...
# The deployed URL retains its historical /console/pty suffix, but this is a
# read-only worker log stream rather than an interactive shell PTY.
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not providers and not _legacy_basic_auth_enabled():
        ...
        _log.info(
            ...
            or f"legacy-basic:{_dashboard_basic_username()}",
        )
