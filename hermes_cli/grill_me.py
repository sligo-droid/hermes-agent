...
def build_grill_me_prompt(user_text: str, runtime_note: str = "") -> str:
    ...
    return (
        ...
        "that are discoverable. Consult repo guidance and decision context when "
        "available, including AGENTS.md, CONTEXT.md, CONTEXT-MAP.md, docs/adr/, "
        "or similar project docs, before asking questions. Ask the user only for "
        "missing decisions, goals, constraints, or tradeoffs that cannot be "
        "inferred.\n\n"
        "When the user's domain terms are fuzzy or overloaded, call that out and "
        "propose precise canonical language. If repository, code, or docs context "
        "can answer a question, inspect that context instead of asking the user.\n\n"
        "Offer ADR or doc capture sparingly: only recommend capturing the decision "
        "when it is hard to reverse, surprising without context, and based on real "
        "tradeoffs. Do not edit docs in grill-me planning mode; offer or recommend "
        "capture only after explicit confirmation.\n\n"
