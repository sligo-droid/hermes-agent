...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Max iterations: 1000")
    print_info("  Tool progress: all")
    print_info("  Compression threshold: 0.50")
    print_info("  Session reset: disabled")
...
def setup_agent_settings(config: dict):
    ...
    current_max = str(cfg_get(config, "agent", "max_turns", default=1000))
    print_info("Maximum tool-calling iterations per conversation.")
    print_info("Higher = more complex tasks, but costs more tokens.")
    print_info(
        f"Press Enter to keep {current_max}. Use 1000 for long-running coding tasks."
    )
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    if section_key == "model":
        ...
        max_turns = cfg_get(config, "agent", "max_turns", default=1000)
...
def run_setup_wizard(args):
    ...
    # recommended defaults silently; existing installs keep their tuned values.
    if not is_existing:
