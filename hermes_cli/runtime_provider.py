...
def _resolve_named_custom_runtime(
    *,
    ...
    if pool_result:
        ...
        if custom_provider.get("key_env"):
            pool_result["key_env"] = custom_provider["key_env"]
        return pool_result
    ...
    declared_key_env = str(custom_provider.get("key_env", "") or "").strip()
    api_key_candidates = [
        (explicit_api_key or "").strip(),
        str(custom_provider.get("api_key", "") or "").strip(),
        _getenv(declared_key_env, "").strip() if declared_key_env else "",
    ]
    api_key = next((candidate for candidate in api_key_candidates if has_usable_secret(candidate)), "")

    # A declared credential source is a trust boundary. If that exact env var
    # is absent, do not borrow unrelated global or host-derived credentials for
    # the endpoint. Explicit keys, inline provider keys, and the matching custom
    # credential pool above are the only permitted alternatives.
    if declared_key_env and not api_key:
        raise AuthError(
            f"Custom provider '{custom_provider.get('name', requested_provider)}' "
            f"requires credential environment variable {declared_key_env}, but it is not set.",
            provider="custom",
    ...
    if declared_key_env:
        result["key_env"] = declared_key_env
...
def resolve_runtime_provider(
    *,
    ...
    # Anthropic has an explicit credential-preference contract: the normal
    # route prefers Claude Code/API-key resolution, while Fable and other
    # scoped callers can request pool_first. Let _resolve_anthropic_runtime()
    # enforce that ordering instead of consuming a generic pool here.
    should_use_pool = provider not in {"openrouter", "anthropic"}
