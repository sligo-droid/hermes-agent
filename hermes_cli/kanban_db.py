...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    """Fire a lifecycle hook after a durable transition, best-effort."""
    try:
...
# Sentinel exit code used by workers that stop because a provider is rate
# limited or out of quota. This is a transient dispatch condition, not a task
# failure, so the crash detector requeues without incrementing the breaker.
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    """Return the configured cooldown for quota-wall requeues."""
    raw = os.environ.get(
...
def _relative_age(ts: Optional[int], now: Optional[int] = None) -> str:
    """Render an epoch timestamp as a coarse age for worker handoffs."""
    if ts is None:
        return ""
    try:
        ts = int(ts)
...
class Task:
    ...
    worker_pid_start_ticks: Optional[int] = None
    ...
    # Goal-loop mode keeps dispatching the task until the worker proves the
    # goal is complete, blocks, or exhausts its turn budget.
    ...
    # blocks. Preserved across unblock so a same-cause re-block can be detected.
    block_kind: Optional[str] = None
    # Unblock-loop counter. Reset only on successful completion.
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            ...
            worker_pid_start_ticks=(
                row["worker_pid_start_ticks"] if "worker_pid_start_ticks" in keys else None
            ),
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional first-class project link. When present on a worktree task,
    ...
    max_retries          INTEGER,
    -- Goal-loop mode. When true, the dispatcher re-runs the worker until it
    -- completes, blocks, is externally reclaimed, or ``goal_max_turns`` is
    -- exhausted. NULL/0 = classic single-shot worker.
    ...
    -- NULL for legacy/un-typed blocks). Preserved across unblock so a re-block
    -- for the same kind can be recognised as a loop.
    block_kind           TEXT,
    -- Unblock-loop counter. Incremented on same-cause re-blocks and reset only
    -- on successful completion, not on unblock.
...
def _cross_process_init_lock(path: Path):
    ...
    Acquisition is bounded so a wedged lock holder cannot stop every future
    board connection indefinitely. On timeout, initialization proceeds under
    the in-process lock; the schema setup is idempotent and remains safe to
...
def _dispatch_tick_lock(db_path: Path):
    """Acquire a non-blocking, board-scoped single-dispatcher file lock."""
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via legacy/worker-handoff
    ...
    # Once this process has completed header/integrity/schema initialization
    # for a path, no cross-process initialization work remains. Skipping the
    # file lock on this steady-state path prevents a stalled first-open in a
    # different process from blocking every dispatcher tick.
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "block_kind" not in cols:
        ...
        # blocks. Existing blocked rows get NULL, preserving old semantics.
        _add_column_if_missing(conn, "tasks", "block_kind", "block_kind TEXT")

    if "block_recurrences" not in cols:
        # Existing rows start at 0; the loop breaker begins counting from the
        # first re-block after this migration.
        _add_column_if_missing(
            conn,
            "tasks",
            "block_recurrences",
...
def create_task(
    ...
    *,
    ...
    # can be named deterministically instead of relying on the worker's random
    # fallback. Projects live in the creator profile's projects DB; the stored
    # repo path/branch are absolute strings, so cross-profile dispatchers do
    # not need projects DB access later.
    project_obj = None
    ...
    if project_id:
        try:
            from hermes_cli import projects_db as project_db_mod

            with project_db_mod.connect_closing() as project_conn:
                project_obj = project_db_mod.get_project(project_conn, project_id)
        except Exception:
            project_obj = None
        if project_obj is None:
            # Do not persist a dangling link or crash creation from a stale UI
            # project id. Fall back to ordinary task creation.
            project_id = None
        else:
            ...
            if (
                ...
                # The concrete path depends on task_id, generated below.
    ...
    # and to avoid holding a write lock during the lookup. Re-check inside
    # the write transaction below so concurrent creators with the same key
    # serialize onto one row instead of forking duplicate work.
    if idempotency_key:
        row = conn.execute(
        ...
        try:
            with write_txn(conn):
                ...
                if project_obj is not None and workspace_kind == "worktree":
                    ...
                    if not branch_name and project_db_mod is not None:
                        try:
                            branch_name = project_db_mod.branch_name_for(
    ...
    with write_txn(conn):
        ...
        for row in todo_rows:
            ...
            if cur_status == "blocked" and _has_sticky_block(conn, task_id):
                # Worker / operator asked for human review; only an explicit
                # unblock event should let this task leave blocked state.
                continue
    ...
    *extended* (with a ``claim_extended`` event) instead of being
    ...
    for row in stale:
        ...
        with write_txn(conn):
            ...
            if guard.rowcount != 1:
                continue
            expected_ticks = row["worker_pid_start_ticks"]
            actual_ticks = payload["worker_pid_start_ticks_actual"]
            pid_identity_mismatch = bool(
                row["worker_pid"] is not None
                and expected_ticks is not None
                and actual_ticks is not None
                and int(expected_ticks) != int(actual_ticks)
            )
            termination = _terminate_reclaimed_worker(
                None if pid_identity_mismatch else row["worker_pid"],
                row["claim_lock"],
                worker_unit=row["worker_unit"],
                signal_fn=signal_fn,
            )
            if pid_identity_mismatch:
                # The stored PID now belongs to another process. Never signal
                # it; the original worker identity is already gone, so release
                # the stale claim while retaining the mismatch in diagnostics.
                termination["prev_pid"] = int(row["worker_pid"])
                termination["pid_identity_mismatch"] = True
...
def _complete_task_once(
    ...
    *,
    ...
    _fire_kanban_lifecycle_hook(
        ...
        task_id,
        board=get_current_board(),
        assignee=completed_task.assignee if completed_task else None,
        run_id=run_id,
        summary=(summary if summary is not None else result),
    )
    return True
...
def _git_toplevel(path: Path) -> Optional[Path]:
    """Return the git toplevel containing ``path``, if any."""
    try:
...
def _git_common_dir(path: Path) -> Optional[Path]:
    ...
    if result.returncode != 0 or not output:
        return None
    return Path(output).expanduser().resolve(strict=False)
...
def _ensure_git_worktree(
    ...
    if result.returncode != 0:
        ...
        raise RuntimeError(
            f"git worktree add failed for {target} on branch "
            f"{branch_name}: {detail}"
        )
...
def _resolve_worktree_workspace(
    ...
    *,
    ...
    """Resolve and materialize a task-specific linked git worktree."""
    ...
    if not task.workspace_path:
        ...
        if not board_default:
            raise ValueError(
                f"task {task.id} has workspace_kind=worktree but no "
        ...
        if not anchor.is_absolute():
            raise ValueError(
                f"board {board_slug!r} default_workdir {board_default!r} "
                "is not absolute"
            )
    ...
    if requested.exists() and _is_linked_worktree_checkout(requested):
        return requested_resolved, _git_current_branch(requested) or branch_name
    ...
    if repo_root is None:
        raise ValueError(
            f"task {task.id} worktree path {task.workspace_path!r} is not "
            "inside a git repo and does not point at a git repo root"
        )
...
def _cleanup_workspace(conn: sqlite3.Connection, task_id: str) -> None:
    ...
    try:
        ...
        if kind != "scratch" or not path:
            # A non-scratch child can still be the final dependency holding a
            # parent's deferred scratch workspace open.
...
def _try_cleanup_parent_workspaces(
    ...
    """Remove deferred parent scratch workspaces once all children finish."""
    try:
        parents = conn.execute(
        ...
        for parent_row in parents:
            ...
            if workspace.is_dir() and _is_managed_scratch_path(workspace):
...
def block_task(
    ...
    *,
    ...
    """Transition ``running``/``ready`` to blocked, todo, or triage.

    ``kind`` distinguishes dependency waits from human blockers and keeps a
    persistent recurrence counter so unblock cron/jobs cannot spin forever on
    ...
    with write_txn(conn):
        ...
        if kind == "dependency":
            params = (kind, task_id) if expected_run_id is None else (kind, task_id, int(expected_run_id))
            cur = conn.execute(
                """
                UPDATE tasks
                   SET status       = 'todo',
        ...
        target_status = "triage" if recurrences >= BLOCK_RECURRENCE_LIMIT else "blocked"
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. Repo-root paths act as
      anchors for ``<repo>/.worktrees/<task-id>``; explicit target paths are
      created or reused. With no path, the board ``default_workdir`` anchors
      the task-specific worktree.
    ...
    if kind == "dir":
        ...
        return p
    if kind == "worktree":
        workspace, _branch_name = _resolve_worktree_workspace(task, board=board)
        return workspace
...
# Space retries after a worker exits through the provider quota sentinel.
...
class DispatchResult:
    ...
    """Task ids requeued after a provider quota-wall exit without failure."""
    ...
    """True when another dispatcher holds this board's tick lock."""
...
def _worker_survived_termination(termination: dict) -> bool:
    """Return whether a managed host-local worker survived our kill attempt."""
    return bool(
        ...
        and (
            ...
            or (
                termination.get("unit_stop_attempted")
                and not termination.get("unit_stopped")
            )
        )
    )
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    """Keep a surviving worker's claim active so no duplicate is spawned."""
...
def check_respawn_guard(conn: sqlite3.Connection, task_id: str) -> Optional[str]:
    ...
    if (
        ...
        if (
            failure_at is None
            or now - failure_at < _RESPAWN_GUARD_AUTH_WINDOW
        ):
            return "blocker_auth"
...
def dispatch_once(
    ...
    *,
    ...
    """Run one dispatcher tick under the board-scoped single-writer lock."""
    try:
    ...
    kwargs = {
        ...
        "board": board,
        "additional_spawnable_assignees": additional_spawnable_assignees,
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        row_assignee_key = str(row_assignee or "").strip().lower()
        if (
            profile_exists is not None
            and row_assignee_key not in additional_spawnable
...
def _default_spawn(
    ...
    *,
    ...
    # The terminal and context-file loaders prefer TERMINAL_CWD over the
    # subprocess cwd. Pin it only when the workspace is a real absolute
    # directory; otherwise preserve any inherited, valid anchor.
