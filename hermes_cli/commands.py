...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("coding-worker", "Toggle coding worker delegation",
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    # Pin high-value aliases ahead of canonical commands so they cannot be
    # silently clamped by Slack's 50-command manifest limit.
    alias_to_cmd = {
        alias: cmd
        for cmd in COMMAND_REGISTRY
        if _is_gateway_available(cmd, overrides, "slack")
        for alias in cmd.aliases
    }
    for alias in _SLACK_PRIORITY_ALIASES:
        cmd = alias_to_cmd.get(alias)
        if cmd is not None:
            _add(
                alias,
...
class SlashCommandCompleter(Completer):
    ...
    def _extract_path_word(text: str) -> str | None:
        ...
        if not word:
            return None
        # URLs contain "/" but are not local paths. Treating them as paths
        # fires filesystem probes on every keystroke while typing a URL.
        if "://" in word:
            return None
    ...
    def _handoff_completions(sub_text: str, sub_lower: str):
        """Yield connected gateway platform completions for /handoff."""
        parts = sub_text.split()
        trailing_space = sub_text.endswith(" ")
        ...
        try:
            ...
            platforms = gateway_config.get_connected_platforms()
        except Exception:
            return
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        ...
        try:
            ...
            # agent.personalities via the CLI config (which ships built-ins).
            # load_config()'s schema has no agent.personalities, so reading it
            # there silently produced only the synthetic ``none`` completion.
            from cli import load_cli_config

            personalities = (
                (load_cli_config().get("agent") or {}).get("personalities", {})
                or {}
            )
    ...
    def get_completions(self, document, complete_event):
        ...
        if len(parts) > 1 or (len(parts) == 1 and text.endswith(" ")):
            ...
            # so it handles both stages itself instead of falling through to
            # the single-word SUBCOMMANDS branch below.
            if base_cmd == "/tools":
