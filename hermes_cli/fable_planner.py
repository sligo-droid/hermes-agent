...
FABLE_REASONING = {"enabled": True, "effort": "xhigh"}
...
FABLE_IMPLEMENTATION_RUNTIME_NOTE = """This is Discord `/fable` implementation mode: use Claude Fable 5 to inspect, coordinate, and review the requested implementation in the normal Discord action-request flow. You may use read-only tools to investigate and review the resulting work. Every repository mutation — including file edits, generated files, dependency changes, tests that write, and git changes — must be performed through `delegate_coding_task` using the Codex coding worker in a mutable git worktree. The Workdir in this request is the pre-provisioned workspace for this turn: pass it as `cwd` when delegating, or omit `cwd` to inherit it; never ask the worker to create a second worktree or clone from the canonical checkout. Ask the worker to stop after local edits and focused verification; it must not stage, commit, push, open or edit PRs, wait for CI, merge, or touch the protected canonical checkout. Trusted Hermes code owns those GitHub lifecycle steps after the worker returns. Do not use write_file, patch, execute_code, or mutating terminal commands to edit the repository yourself. Do not fall back to OpenCode, Claude Code, or direct Fable edits. If Codex or a mutable worktree is unavailable, report that exact blocker clearly. The Git Lifecycle Policy is gateway-owned, not model-controlled; report the deterministic `fable_git_result` returned by the worker tool and do not claim remote Git actions without that evidence. After the worker returns, inspect and review its result, run only safe read-only verification yourself, and report the outcome concisely."""
