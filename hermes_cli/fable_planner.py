...
def fable_reasoning_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    """Return Fable's fixed reasoning level.

    Fable is a dedicated premium route with a deliberately pinned reasoning
    ...
    promoted every fable turn). ``config`` is accepted for call-site
