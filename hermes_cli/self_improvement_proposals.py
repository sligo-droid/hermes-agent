...
def proposal_json_block_reference(project_key: str, prong_key: str) -> str:
    ...
    return (
        "Always include exactly one JSON block named "
        f"{PROPOSAL_BLOCK_KEY!r}. Use an empty proposals list when there is no self-improvement work. "
        "Do not infer proposals outside this block. "
        "Use this shape:\n"
        "```json\n"
        + json.dumps(
            {
...
def ingest_cron_proposal_output(
    ...
    *,
    ...
    cfg = config or load_config()
    try:
        scope = resolve_cron_project_prong(job, config=cfg)
    ...
    try:
        ...
        block_project = text(block.get("project")).strip()
        block_prong = text(block.get("prong")).strip()
        if block_project or block_prong:
            if not block_project:
                raise ValueError("proposal block field 'project' is required when 'prong' is supplied")
            try:
                block_scope = resolve_project_prong(block_project, block_prong or None, config=cfg)
            except KeyError as exc:
                raise ValueError(f"proposal block scope is not configured: {block_project}/{block_prong}") from exc
            if block_scope["project_key"] != scope["project_key"] or block_scope["prong_key"] != scope["prong_key"]:
                raise ValueError(
                    "proposal block scope "
                    f"{block_scope['project_key']}/{block_scope['prong_key']} does not match configured cron scope "
                    f"{scope['project_key']}/{scope['prong_key']}"
                )
            parser_metadata.update({"project": block_scope["project_key"], "prong": block_scope["prong_key"]})
        ...
        missing_block = str(exc) == "missing required proposal JSON block"
        ...
        if legacy_cards:
            ...
            parse_error = str(exc)
            cards = legacy_cards
        elif missing_block and not _has_legacy_proposal_markers(raw_text):
            parser_metadata.update({"empty_reason": "missing_block_no_proposal_markers"})
            cards = []
        else:
            parse_status = "failed"
            parse_error = str(exc)
...
def list_cards(
    *,
    ...
    def op(c: sqlite3.Connection) -> list[dict[str, Any]]:
        clauses = []
        params: list[Any] = []
        for col, val in (("run_id", run_id), ("project_key", project_key), ("prong_key", prong_key), ("status", status)):
...
def transition_card(
    ...
    *,
    ...
    def op(c: sqlite3.Connection) -> dict[str, Any]:
        ...
        if card.get("status") == status and status in {"approved", "rejected"}:
            return card
        now = utc_now()
...
def _has_legacy_proposal_markers(output_text: str) -> bool:
    if PROPOSAL_BLOCK_KEY in output_text:
        return True
    return bool(re.search(r"^#{2,4}\s+Proposal\s*[:\-]", output_text, flags=re.IGNORECASE | re.MULTILINE))
