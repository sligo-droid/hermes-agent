...
def _extract_json_candidates(text: str) -> list[Any]:
    ...
    for match in re.finditer(r"```(?:json|self-improvement-proposals)?\s*(\{.*?\})\s*```", text, re.DOTALL | re.IGNORECASE):
    ...
    if markers:
        candidates.insert(0, markers.group(1))
    parsed: list[Any] = []
...
def parse_proposal_payload(source_text: str, proposal_json: Optional[dict[str, Any]] = None) -> dict[str, Any]:
    ...
    The preferred contract is a JSON object with ``project``, ``prong`` and
    ``cards``.  Markdown source is accepted only when it carries that object in
    a fenced JSON or BEGIN/END marker block; arbitrary prose splitting remains
...
def _coerce_card(raw: dict[str, Any], *, run_id: str, project: ProjectConfig, prong: ProngConfig, index: int) -> dict[str, Any]:
    ...
    return {
        ...
        "title": _redact(title) or title,
        "summary": _truncate(_redact(raw.get("summary") or ""), 1200),
        "body": _truncate(_redact(raw.get("body") or raw.get("rationale") or ""), 8000),
        "evidence_json": _json_dumps([_redact(str(item)) for item in evidence if item is not None]),
        "recommended_action": _truncate(_redact(raw.get("recommended_action") or ""), 2000),
        "worker_prompt": _truncate(_redact(raw.get("worker_prompt") or raw.get("prompt") or raw.get("recommended_action") or title), 8000),
        "acceptance_criteria_json": _json_dumps([_redact(str(item)) for item in criteria if item is not None]),
        "risk_notes": _truncate(_redact(raw.get("risk_notes") or raw.get("risk") or ""), 2000),
        "priority": priority_i,
