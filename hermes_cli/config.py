...
from hermes_cli.fork_defaults import apply_fork_defaults
...
DEFAULT_CONFIG = {
    ...
    "display": {
        ...
        "busy_input_mode": "steer",  # steer | queue | interrupt
    ...
    "kanban": {
        ...
        # session with a persistent delivery channel.
...
# defaults isolated from the fast-moving upstream schema. Missing keys are
# added without replacing upstream additions; the small explicit override set
# documents behavior this fork intentionally keeps authoritative.
apply_fork_defaults(DEFAULT_CONFIG)
...
def _persist_migration(
    ...
    *,
    ...
    calling ``save_config`` directly. It keeps default-stripping ON and does
    not merge the existing document;
    ...
    """
    # Two historical migrations intentionally materialized schema defaults so
    # users could discover the new gates in their existing config. Preserve
    # them on every later migration write once present; otherwise a subsequent
    # version step would strip them again because empty dicts have no leaf path.
    migration_materialized_paths = {
        ("display", "interim_assistant_messages"),
        ("discord", "channel_prompts"),
    }
    save_config(
        config,
        preserve_keys=migration_materialized_paths | set(preserve_keys or ()),
    )
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 15:
        ...
        if "interim_assistant_messages" not in display:
            ...
            _persist_migration(
                config,
                preserve_keys={("display", "interim_assistant_messages")},
            )
            if not quiet:
    ...
    # ── Version 17 → 18: materialize Discord channel prompts ──
    # The empty mapping is intentionally written once so existing users can see
    # where per-channel prompts belong. Empty dicts have no leaf path for the
    # normal explicit-key detector, so preserve this migration-owned path.
    if current_ver < 18:
        config = read_raw_config()
        discord_config = config.get("discord")
        if not isinstance(discord_config, dict):
            discord_config = {}
        if "channel_prompts" not in discord_config:
            discord_config["channel_prompts"] = {}
            config["discord"] = discord_config
            _persist_migration(
                config,
                preserve_keys={("discord", "channel_prompts")},
            )
            results["config_added"].append("discord.channel_prompts")
            if not quiet:
    ...
    # Only the exact persisted former default is rewritten. Custom tier names,
    # disabled values, and differently-cased strings remain untouched.
    if current_ver < 26:
        config = read_raw_config()
        discord_config = config.get("discord")
        if (
