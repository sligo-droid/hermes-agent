...
def _probe_single_server(
    ...
    *,
    ...
    if connect_timeout is None and overall_timeout is None:
    ...
    async def _probe():
        ...
        if connect_timeout is None:
            ...
            server = await asyncio.wait_for(connect_coro, timeout=connect_timeout)
        try:
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    # Disable the inner connection timeout and bound the complete interactive
    # login flow to five minutes so a human has time to finish browser auth.
    try:
