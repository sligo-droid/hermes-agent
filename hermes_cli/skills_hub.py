...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    _PER_SOURCE_LIMIT = {
        "official": 200, "hermeshub": 200, "skills-sh": 200, "well-known": 50,
    ...
    for r in all_results:
        ...
        key = getattr(r, "identifier", None) or r.name
        if key not in seen or rank > _TRUST_RANK.get(seen[key].trust_level, 0):
            seen[key] = r
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    _PER_SOURCE_LIMIT = {"official": 100, "hermeshub": 100, "skills-sh": 100, "well-known": 25, "github": 100, "clawhub": 50,
...
def handle_skills_slash(cmd: str, console: Optional[Console] = None) -> None:
    ...
    if action == "browse":
        ...
        if not args:
            c.print("[bold red]Usage:[/] /skills search <query> [--source hermeshub|skills-sh|well-known|github|official] [--limit N] [--json]\n")
            return
        source = "all"
        limit = 10
        as_json = False
