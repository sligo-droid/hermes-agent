...
START_PROMPT_WITH_CONTRACT_TEMPLATE = (
    "[Starting work toward your standing goal]\n"
    "Goal: {goal}\n\n"
    "Completion contract:\n{contract_block}\n\n"
    "Work toward the outcome above, stay within its boundaries, and prove "
    "completion with the specified verification evidence."
)
...
JUDGE_SYSTEM_PROMPT = (
    ...
    "BLOCKED — the goal is not done and cannot continue without user input, "
    "credentials, external access, or missing goal state. Do not call a "
    "blocked goal DONE.\n\n"
...
JUDGE_USER_PROMPT_WITH_CONTRACT_TEMPLATE = (
    ...
    "Is the goal satisfied per its completion contract — done, continue, wait, or blocked?"
)
...
class GoalState:
    ...
    def from_json(cls, raw: str) -> "GoalState":
        ...
        return cls(
            ...
            subgoals=subgoals,
            runtime_breakdown=(
                data.get("runtime_breakdown")
                if isinstance(data.get("runtime_breakdown"), dict)
                else {}
            ),
...
def agent_goal_context(session_id: str) -> str:
    """Return bounded API-only context for the live goal state."""
    state = load_goal(session_id)
    ...
    if state.subgoals:
        lines.extend(("Subgoals:", _truncate(state.render_subgoals_block(), 2000)))
    if state.has_contract():
        lines.extend(("Completion contract:", _truncate(state.contract.render_block(), 2500)))
    lines.append(
...
def _parse_judge_response(raw: str) -> Tuple[str, str, bool, Optional[Dict[str, Any]]]:
    ...
    if not isinstance(verdict_raw, str):
        verdict_raw = data.get("status")
    if isinstance(verdict_raw, str):
        verdict = verdict_raw.strip().lower()
    else:
        if data.get("blocked") is True:
            return "blocked", reason, False, None
...
def _parse_judge_verdict(raw: str) -> Tuple[str, str, bool]:
    """Compatibility view of the structured judge parser without wait data."""
    verdict, reason, parse_failed, _wait_directive = _parse_judge_response(raw)
...
class GoalManager:
        ...
        *,
        ...
        if isinstance(runtime_breakdown, dict) and runtime_breakdown:
            try:
                ...
                existing = (
                    state.runtime_breakdown
                    if isinstance(state.runtime_breakdown, dict)
                    else {}
                )
    ...
    def initial_work_prompt(self) -> Optional[str]:
        """Return the first user-role prompt for a newly activated goal."""
        if not self._state or self._state.status != "active":
            return None
        if self._state.has_contract():
            contract_block = self._state.contract.render_block()
            if self._state.subgoals:
                ...
                contract_block = f"{contract_block}\n{extra}"
            return START_PROMPT_WITH_CONTRACT_TEMPLATE.format(
                goal=self._state.goal,
                contract_block=contract_block,
            )
        if self._state.subgoals:
