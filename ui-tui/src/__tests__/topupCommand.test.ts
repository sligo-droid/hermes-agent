...
describe('/billing slash command (overlay-driven)', () => {
  ...
  it('keeps the legacy /billing and /credits spellings on the single /topup handler', () => {
    expect(findSlashCommand('billing')).toBe(topupCommand)
    expect(findSlashCommand('credits')).toBe(topupCommand)
    expect(findSlashCommand('topup')).toBe(topupCommand)
  })
