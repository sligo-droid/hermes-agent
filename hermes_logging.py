...
from logging.handlers import (
    QueueHandler,
    QueueListener,
    RotatingFileHandler as _StdlibRotatingFileHandler,
)
from pathlib import Path
from typing import Optional, Sequence

# Windows cannot rename an open log file during rollover. Hermes normally has
# several processes writing the same logs, so use the upstream cross-process
# rotator there while retaining stdlib's handler semantics on POSIX.
if sys.platform == "win32":
    from concurrent_log_handler import (  # noqa: E402
        ConcurrentRotatingFileHandler as RotatingFileHandler,
    )
else:
    RotatingFileHandler = _StdlibRotatingFileHandler
...
def _safe_stderr():  # type: ignore[return]
    """Return a stderr stream that tolerates Unicode on legacy consoles."""
    stream = sys.stderr
...
def _is_windows_concurrent_log_lock_timeout(exc: BaseException | None) -> bool:
    """Return True only for the known Windows concurrent-rotator timeout."""
    return (
        sys.platform == "win32"
...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    if (
        not force
        and _has_managed_log_handlers()
        and not _managed_handlers_target_log_dir(log_dir)
    ):
        # A stale sentinel is not required for rehome detection: embedded
        # callers and tests sometimes reset it directly while handlers remain.
        force = True

    if _logging_initialized and not force:
        if mode in {"gateway", "gui"}:
            # Component startup can follow earlier CLI logging setup in the
            # same process; attach its filtered log without rebuilding the
            # catch-all handlers.
            ...
            _add_rotating_handler(
                ...
                log_dir / f"{mode}.log",
                level=logging.INFO,
                max_bytes=(10 if mode == "gui" else 5) * 1024 * 1024,
                backup_count=5 if mode == "gui" else 3,
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    def handleError(self, record: logging.LogRecord) -> None:
        """Keep the known Windows cross-process lock timeout inside logging."""
...
def _is_rotating_file_handler(handler: logging.Handler) -> bool:
    """Match both stdlib and Windows concurrent rotating handlers."""
    return isinstance(handler, (_StdlibRotatingFileHandler, RotatingFileHandler))
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        root = logging.getLogger()
        # Detach producers first so no record can be enqueued behind the stop
        # sentinel while the listener drains the existing queue.
...
def shutdown_logging() -> None:
    """Flush, detach, and close every Hermes-managed file handler.

    This is safe to call repeatedly and is the supported lifecycle boundary
    for profile reconfiguration and embedded/test processes. It deliberately
    removes both async queue targets and older synchronous rotating handlers.
    """
    global _logging_initialized
    _reset_queued_handlers()
    _remove_managed_rotating_handlers(logging.getLogger())
    _logging_initialized = False


def _managed_handlers_target_log_dir(log_dir: Path) -> bool:
    """Return True when all live Hermes file handlers target *log_dir*."""
    expected = log_dir.resolve()
    managed = _managed_log_handlers()
    if not managed:
        return False
    try:
        return all(Path(h.baseFilename).resolve().parent == expected for h in managed)
    except (AttributeError, TypeError, ValueError):
        return False


def _managed_log_handlers() -> list[logging.Handler]:
    """Snapshot all async and legacy synchronous Hermes file handlers."""
    with _queue_state_lock:
        handlers = list(_queued_file_handlers) + list(logging.getLogger().handlers)
    return [h for h in handlers if _is_hermes_log_handler(h)]


def _has_managed_log_handlers() -> bool:
    return bool(_managed_log_handlers())
    ...
    *,
    ...
    with _queue_state_lock:
        for existing in _queued_file_handlers:
            if (
                _is_rotating_file_handler(existing)
        ...
        # Route through the async queue instead of ``logger.addHandler(handler)``
        # so rotation-lock waits never run on an event-loop thread.
...
def _remove_managed_rotating_handlers(logger: logging.Logger) -> None:
    ...
    for handler in list(logger.handlers):
        if _is_rotating_file_handler(handler) and _is_hermes_log_handler(handler):
            logger.removeHandler(handler)
            handler.close()


def _is_hermes_log_handler(handler: logging.Handler) -> bool:
    """Return True for handlers targeting Hermes-managed log files."""
    try:
        return _is_rotating_file_handler(handler) and Path(handler.baseFilename).name in {
            "agent.log",
            "errors.log",
            "gateway.log",
            "gui.log",
