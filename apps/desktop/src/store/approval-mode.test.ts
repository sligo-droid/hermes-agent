...
describe('profile-scoped approval mode cache', () => {
  ...
  it('labels an unread profile Manual by default and adopts backend truth', async () => {
  ...
  it('preserves configured Smart mode from the backend', async () => {
  ...
  it('rolls a failed first write back to Manual', async () => {
    const request = vi.fn(async () => {
      throw new Error('write failed')
    })

    await expect(setApprovalModeForProfile(request, 'fresh-write', 'off')).rejects.toThrow('write failed')

    expect(approvalModeForProfile('fresh-write')).toBe('manual')
  })
