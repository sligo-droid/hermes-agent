...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def _format_first_turn_context(self, ctx: dict) -> str:
        """Format bounded, high-signal Honcho context for prompt injection."""
        parts = []
        sections = (
            ("summary", "## Session Summary", ctx.get("summary", ""), False),
            ("card", "## User Peer Card", ctx.get("card", ""), True),
            ("ai_card", "## AI Identity Card", ctx.get("ai_card", ""), True),
        )
        for key, heading, value, is_card in sections:
            cleaned = self._clean_context_section(key, value, is_card=is_card)
            if cleaned:
                parts.append(f"{heading}\n{cleaned}")
        return "\n\n".join(parts)

    def _clean_context_section(
        self, key: str, value: Any, *, is_card: bool = False,
    ) -> str:
