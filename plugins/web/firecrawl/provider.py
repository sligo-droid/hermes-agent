...
class FirecrawlWebSearchProvider(WebSearchProvider):
    ...
    async def crawl(self, url: str, **kwargs: Any) -> Dict[str, Any]:
        """Crawl a site via Firecrawl and return the legacy crawl result shape."""
        from tools.interrupt import is_interrupted as _is_interrupted
        ...
        try:
            crawl_result = await asyncio.wait_for(
                asyncio.to_thread(_get_firecrawl_client().crawl, url, **kwargs),
                timeout=60,
            )
        except Exception as exc:  # noqa: BLE001
            return {"results": [{"url": url, "title": "", "content": "", "error": str(exc)}]}

        pages = _to_plain_object(crawl_result).get("data", [])
        results: List[Dict[str, Any]] = []
        for page in pages:
            page = _to_plain_object(page)
            metadata = _to_plain_object(page.get("metadata", {}))
            ...
            if blocked:
                ...
                continue
            content = page.get("markdown") or page.get("html") or page.get("content") or ""
            results.append(
                {
                    "url": final_url,
                    "title": title,
