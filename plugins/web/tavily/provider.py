...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def crawl(self, url: str, **kwargs: Any) -> Dict[str, Any]:
        """Crawl a site via Tavily and return the legacy crawl result shape."""
        try:
            from tools.interrupt import is_interrupted
            ...
            return {"results": _normalize_tavily_documents(raw, fallback_url=url)}
        except ValueError as exc:
            return {"results": [{"url": url, "title": "", "content": "", "error": str(exc)}]}
        except Exception as exc:  # noqa: BLE001
            logger.warning("Tavily crawl error: %s", exc)
            return {"results": [{"url": url, "title": "", "content": "", "error": f"Tavily crawl failed: {exc}"}]}
