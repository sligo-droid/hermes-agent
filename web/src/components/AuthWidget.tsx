...
export function AuthWidget({ className }: AuthWidgetProps) {
  const authRequired = Boolean(window.__HERMES_AUTH_REQUIRED__);
  ...
  useEffect(() => {
    ...
    api
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        // 401/403 from /api/auth/me means the gate state changed after
        // the page loaded or the session vanished; render nothing rather
        // than treating the sidebar identity probe as a page-level error.
