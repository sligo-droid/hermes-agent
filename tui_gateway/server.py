...
def _load_approval_mode() -> str:
    ...
    raw_cfg = _load_cfg()
    approvals = raw_cfg.get("approvals") if isinstance(raw_cfg, dict) else None
    raw = approvals.get("mode", "manual") if isinstance(approvals, dict) else "manual"
...
def _persist_model_switch(result) -> None:
    ...
    config_root = getattr(cli_mod, "_hermes_home", None)
    config_path = config_root / "config.yaml" if config_root is not None else None
    ...
    def _save(key: str, value: Any) -> None:
        if (
            ...
            and "<lambda>" not in repr(save_fn)
        ):
